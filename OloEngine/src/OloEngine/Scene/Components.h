...
namespace OloEngine
{
    ...
    struct LODGroupComponent
    {
        ...
        // assets, whose handles cannot survive a process restart, so the scene
        // serializer treats such a group as DERIVED data: it persists this flag and
        // regenerates the levels on load rather than writing handles nothing can
        // resolve. An authored group is persisted level by level as before.
        //
        // OLO_SERIALIZE(Skip) is here for its OTHER effect: this component's scene
        // (de)serialize is hand-written (kComponentsCustomSerialize), so the tag
        // changes nothing there — but the shared field scan also feeds the MCP
        // writable-field registry, and flipping this flag true from a debug tool
        // would make the next save DROP an authored group's levels. It is a
        // provenance record, not a knob.
        OLO_SERIALIZE(Skip)
        bool m_AutoGenerated = false;

        // The level resolved for this entity's SHARED ANIMATED SURFACE, decided once
        // per frame at the frame boundary by Scene::SelectAnimatedSurfaceLOD (#1227)
        // and then used by BOTH the morph deformation pass and the animated draw
        // loop. Deciding it once is the whole point: a static mesh can select its
        // level inside submission because nothing deforms it first, but a skinned or
        // morphing entity has a CPU deformation pass that must write to the same mesh
        // the renderer is about to draw. Selecting in two places would let the morph
        // pass deform LOD 0 while the renderer drew LOD 2.
        //
        // -1 means "no animated selection this frame" — the entity is not animated,
        // the group is disabled, or selection found nothing.
        //
        // Transient — excluded from copy & equality, like m_GeneratedLODHandles.
        OLO_SERIALIZE(Skip)
        i32 m_ActiveAnimatedLOD = -1;
        OLO_SERIALIZE(Skip)
        Ref<Mesh> m_ActiveAnimatedMesh;
        // The level last REFUSED for carrying no bone stream, so that refusal is
        // logged once per level rather than once per frame. Not m_ActiveAnimatedLOD:
        // a refusal writes -1 into that, so it can never equal the refused level.
        OLO_SERIALIZE(Skip)
        i32 m_RefusedAnimatedLOD = -1;

        LODGroupComponent() = default;
        LODGroupComponent(const LODGroupComponent& other)
            : m_LODGroup(other.m_LODGroup), m_Enabled(other.m_Enabled), m_AutoGenerated(other.m_AutoGenerated)
        {
        }
        LODGroupComponent& operator=(const LODGroupComponent& other)
        {
            if (this != &other)
            {
                ...
                m_Enabled = other.m_Enabled;
                m_AutoGenerated = other.m_AutoGenerated;
            }
            return *this;
        }
