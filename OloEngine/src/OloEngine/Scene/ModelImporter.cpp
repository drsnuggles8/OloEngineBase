...
namespace OloEngine
{
        ...
        bool resetPlaybackState)
    {
        ...
        // Automatic LOD chain (issue #711). Two conditions, both load-bearing:
        //
        //   * `resetPlaybackState` is what separates a FRESH IMPORT from a
        //     deserialize (SceneSerializer passes false when it re-wires an entity
        //     from a saved scene). Generating there would add a component the scene
        //     file never had, cook every mesh on every scene load, and then persist
        //     the invented group on the next save.
        //   * animated sources are skipped outright — the simplifier drops bone
        //     weights and morph deltas, so a skinned LOD renders in bind pose.
        if (resetPlaybackState && !result.IsAnimated)
        {
    ...
    bool ModelImporter::PopulateStaticEntity(Entity entity, const Ref<Model>& model)
    {
        ...
        // Automatic LOD chain (issue #711) — see EnsureAutoLODGroup.
    ...
    bool ModelImporter::EnsureAutoLODGroup(Entity entity)
    {
        ...
        {
            // An AUTHORED group always wins — never overwrite one.
            //
            // A previously GENERATED one must be replaced rather than kept, because
            // a re-import swaps m_MeshSource underneath it: the editor's "Import
            // Model" buttons assign the new mesh onto the existing entity, and a
            // stale chain would keep drawing the PREVIOUS model's geometry at every
            // level past LOD 0. Its memory-only assets are released here — nothing
            // else will, and the entity is about to stop referencing them.
            if (!entity.GetComponent<LODGroupComponent>().m_AutoGenerated)
            {
                return false;
            }
            ReleaseGeneratedLODAssets(entity.GetComponent<LODGroupComponent>());
