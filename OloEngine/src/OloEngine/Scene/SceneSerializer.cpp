...
#define READ_SCRIPT_FIELD(FieldType, Type)               \
                                            ...
                                            std::unordered_map<std::string, Ref<AnimatedModel>>& animatedModelCache)
    {
        ...
        {
            // Authored settings are restored either way; only the LEVELS differ
            // between an authored group and a regenerated one.
            bool lodEnabled = true;
            if (lodGroupComponent["Enabled"])
            {
                lodEnabled = lodGroupComponent["Enabled"].as<bool>();
            }
            f32 lodBias = 1.0f;
            if (lodGroupComponent["Bias"])
            {
                f32 const bias = lodGroupComponent["Bias"].as<f32>();
            ...
            // A group the importer generated (issue #711) carries no levels on disk —
            // they name memory-only Mesh assets that died with the authoring process.
            // Rebuild it from the MeshComponent this loop already restored above.
            bool const autoGenerated =
                lodGroupComponent["AutoGenerated"] && lodGroupComponent["AutoGenerated"].as<bool>();
            if (autoGenerated)
            {
                // Regeneration can decline — auto-LOD off, no graphics device, or a
                // mesh the simplifier cannot reduce. The component is still added in
                // that case, empty: it carries the authored Enabled/Bias AND the
                // derived-data marker forward, so re-saving from a headless tool does
                // not silently strip the entity's LOD group for everyone. An empty
                // group is inert — SelectLODMesh returns early and the base mesh
                // draws at full detail.
                // HasComponent first: EnsureAutoLODGroup also returns false when it
                // found an existing AUTHORED group and declined to touch it, and
                // AddComponent asserts on a duplicate.
                if (!ModelImporter::EnsureAutoLODGroup(deserializedEntity) &&
                    !deserializedEntity.HasComponent<LODGroupComponent>())
                {
                    deserializedEntity.AddComponent<LODGroupComponent>();
                }
                auto& regenerated = deserializedEntity.GetComponent<LODGroupComponent>();
                regenerated.m_AutoGenerated = true;
                regenerated.m_Enabled = lodEnabled;
                regenerated.m_LODGroup.Bias = lodBias;
            }
            else
            {
                auto& lodComp = deserializedEntity.AddComponent<LODGroupComponent>();
                lodComp.m_Enabled = lodEnabled;
                lodComp.m_LODGroup.Bias = lodBias;
                if (lodGroupComponent["Levels"])
                {
                    for (auto levelNode : lodGroupComponent["Levels"])
                    {
                        ...
                        if (levelNode["Error"])
                        {
                            // Generated per-level error (issue #711). Absent on scenes
                            // saved before it existed and on hand-authored groups, which
                            // is exactly what keeps those on the distance path.
                            f32 const error = levelNode["Error"].as<f32>();
...
#include "OloEngine/Scene/Generated/SceneDeserializeComponents.Generated.inl"
    ...
    void SceneSerializer::SerializeEntity(YAML::Emitter& out, Entity entity)
    {
        ...
        if (entity.HasComponent<LODGroupComponent>())
        {
            ...
            // A level naming a MEMORY-ONLY Mesh asset cannot be persisted: that handle
            // is dead the moment the process exits, and storing a name nothing can
            // resolve is the archetype in cache-stored-unresolvable-reference.md —
            // whose failure only shows on the SECOND load, which no CI run performs.
            //
            // The test is the HANDLES, not the m_AutoGenerated intent flag. A chain
            // that started generated and was then hand-edited still holds generated
            // levels, so trusting the flag alone would write exactly the dead handles
            // this avoids. Anything unpersistable is written as a derived group
            // instead, and the loader regenerates it.
            // HasAssetManager() first: AssetManager::IsMemoryAsset asserts without
            // one, and a headless serialize with no asset system has no memory-only
            // assets to find anyway.
            // Two independent facts, and a group is derived if EITHER holds:
            //
            //   m_AutoGenerated   — PROVENANCE. Set by the generator, cleared by the
            //                       inspector when a level is re-pointed at a real
            //                       asset. Still true after the chain's memory-only
            //                       assets have been released, which is the case no
            //                       handle check can see.
            //   hasMemoryOnlyLevel — PERSISTABILITY. A memory-only handle is dead the
            //                       moment the process exits, so writing it stores a
            //                       reference nothing can ever resolve
            //                       (cache-stored-unresolvable-reference.md).
            //
            // Neither subsumes the other: dropping the flag would mis-persist a
            // released chain as authored, and dropping the handle check would write
            // dead handles for a half-edited one. `HasAssetManager()` first because
            // AssetManager::IsMemoryAsset asserts without one.
            bool const hasMemoryOnlyLevel =
                Project::HasAssetManager() &&
                std::ranges::any_of(lodComp.m_LODGroup.Levels,
                                    [](const LODLevel& level)
                                    { return level.MeshHandle != 0 && AssetManager::IsMemoryAsset(level.MeshHandle); });
            bool const derived = lodComp.m_AutoGenerated || hasMemoryOnlyLevel;

            out << YAML::Key << "Enabled" << YAML::Value << lodComp.m_Enabled;
            out << YAML::Key << "Bias" << YAML::Value << lodComp.m_LODGroup.Bias;
            out << YAML::Key << "AutoGenerated" << YAML::Value << derived;
            ...
            if (!derived)
            {
                ...
                for (const auto& level : lodComp.m_LODGroup.Levels)
                {
                    ...
                    out << YAML::Key << "Error" << YAML::Value << level.Error;
                    out << YAML::EndMap;
                }
                out << YAML::EndSeq;
            }
