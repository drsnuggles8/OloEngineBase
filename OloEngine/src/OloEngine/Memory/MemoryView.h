...
public:
	...
	constexpr inline TMemoryView(const TMemoryView<OtherDataType>& InView)
		: m_Data(InView.m_Data)
		, m_Size(InView.m_Size)
	{
	}
	...
	inline TMemoryView(DataType* InData, DataEndType* InDataEnd)
		: m_Data(InData)
		, m_Size(static_cast<u64>(static_cast<ByteType*>(static_cast<DataType*>(InDataEnd)) - static_cast<ByteType*>(InData)))
	{
	}

	// Returns a pointer to the start of the view.
	[[nodiscard]] constexpr inline DataType* GetData() const { return m_Data; }

	// Returns a pointer to the end of the view.
	[[nodiscard]] inline DataType* GetDataEnd() const { return GetDataAtOffsetNoCheck(m_Size); }

	// Returns the number of bytes in the view.
	[[nodiscard]] constexpr inline u64 GetSize() const { return m_Size; }

	// Returns whether the view has a size of 0 regardless of its data pointer.
	[[nodiscard]] constexpr inline bool IsEmpty() const { return m_Size == 0; }
	...
	[[nodiscard]] inline i32 CompareBytes(const TMemoryView<OtherDataType>& InView) const
	{
		...
		return Compare || m_Size == InView.m_Size ? Compare : m_Size < InView.m_Size ? -1 : 1;
	}
