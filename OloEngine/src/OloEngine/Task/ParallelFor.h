            ...
            const TArrayView<ContextType>& Contexts)
        {
                  ...
                  private FInheritedContextBase
            {
                ...
                FParallelForData(
                    ...
                    : DebugName(InDebugName), Num(InNum), BatchSize(InBatchSize), NumBatches(InNumBatches), Contexts(InContexts), Body(InBody), FinishedSignal(InFinishedSignal), Priority(InPriority)
                {
            ...
            class FParallelExecutor
            {
              ...
              public:
                inline FParallelExecutor(FDataHandle&& InData, i32 InWorkerIndex)
                    : Data(MoveTemp(InData)), WorkerIndex(InWorkerIndex)
                {
                }
