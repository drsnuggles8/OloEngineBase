	...
	namespace Private
	{
		...
		protected:
			...
			void Init(const char* InDebugName, ETaskPriority InPriority, EExtendedTaskPriority InExtendedPriority, ETaskFlags Flags)
			{
				...
				m_LowLevelTask.Init(InDebugName, InPriority,
					[this, Deleter = LowLevelTasks::TDeleter<FTaskBase, &FTaskBase::Release>{ this }]()
					{
						TryExecuteTask();
					},
...
// @brief Try to set the execution flag atomically
		...
		private:
			...
			void Schedule(bool& bWakeUpWorker)
			{
				...
				LowLevelTasks::EQueuePreference QueuePref = bWakeUpWorker ? 
