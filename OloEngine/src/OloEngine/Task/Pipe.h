    ...
    class FPipe
    {
      public:
        ...
        explicit FPipe(const char* InDebugName)
            : m_EmptyEventRef(std::make_shared<FEventCount>()), m_DebugName(InDebugName)
        {
        }
        ...
        bool WaitUntilEmpty(FMonotonicTimeSpan InTimeout = FMonotonicTimeSpan::Infinity())
        {
            ...
            while (true)
            {
                ...
                FMonotonicTimeSpan WaitTime = Timeout.WillNeverExpire() ? FMonotonicTimeSpan::Infinity() : Timeout.GetRemainingTime();
