...
namespace OloEngine
{
	...
	public:
		...
		[[nodiscard]] const Ref<Texture2DArray>& GetCSMTextureArray() const { return m_CSMTextureArray; }
		[[nodiscard]] const Ref<Texture2DArray>& GetSpotTextureArray() const { return m_SpotTextureArray; }
		...
		[[nodiscard]] const glm::mat4& GetCSMMatrix(u32 cascade) const { return m_UBOData.DirectionalLightSpaceMatrices[cascade]; }
		[[nodiscard]] const glm::vec4& GetCascadePlaneDistances() const { return m_UBOData.CascadePlaneDistances; }
		[[nodiscard]] const glm::mat4& GetSpotMatrix(u32 index) const { return m_UBOData.SpotLightSpaceMatrices[index]; }
		[[nodiscard]] i32 GetSpotShadowCount() const { return m_UBOData.SpotShadowCount; }
		[[nodiscard]] i32 GetPointShadowCount() const { return m_UBOData.PointShadowCount; }
		[[nodiscard]] const glm::mat4& GetPointFaceMatrix(u32 lightIndex, u32 face) const { return m_PointLightFaceMatrices[lightIndex][face]; }
		[[nodiscard]] const glm::vec4& GetPointShadowParams(u32 index) const { return m_UBOData.PointLightShadowParams[index]; }

		[[nodiscard]] u32 GetResolution() const { return m_Settings.Resolution; }
		[[nodiscard]] const ShadowSettings& GetSettings() const { return m_Settings; }
		void SetSettings(const ShadowSettings& settings);

		[[nodiscard]] bool IsEnabled() const { return m_Settings.Enabled; }
		void SetEnabled(bool enabled) { m_Settings.Enabled = enabled; }

		void SetDirectionalShadowEnabled(bool enabled) { m_UBOData.DirectionalShadowEnabled = enabled ? 1 : 0; }
		void SetSpotShadowCount(i32 count) { m_UBOData.SpotShadowCount = count; }
		void SetPointShadowCount(i32 count) { m_UBOData.PointShadowCount = count; }
		void SetCascadeDebugEnabled(bool enabled) { m_UBOData.CascadeDebugEnabled = enabled ? 1 : 0; }
		[[nodiscard]] bool IsCascadeDebugEnabled() const { return m_UBOData.CascadeDebugEnabled != 0; }

		// Reset per-frame state (call at BeginScene)
		void BeginFrame();

		// Accessors for shadow-pass rendering UBOs (shared across frames)
		[[nodiscard]] Ref<UniformBuffer>& GetShadowCameraUBO() { return m_ShadowCameraUBO; }
		[[nodiscard]] Ref<UniformBuffer>& GetShadowModelUBO() { return m_ShadowModelUBO; }
		[[nodiscard]] Ref<UniformBuffer>& GetShadowAnimationUBO() { return m_ShadowAnimationUBO; }
