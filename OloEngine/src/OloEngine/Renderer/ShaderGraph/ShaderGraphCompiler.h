...
namespace OloEngine
{
    ...
    {
        FString Name;            // Uniform name (e.g., "u_Albedo")
        ShaderGraphPinType Type; // Data type
        ShaderGraphPinValue DefaultValue;
    };

    template<>
    struct TIsTriviallyRelocatable<ShaderGraphParameterInfo>
    {
        static constexpr bool Value = TIsTriviallyRelocatable_V<decltype(ShaderGraphParameterInfo::Name)> &&
                                      TIsTriviallyRelocatable_V<decltype(ShaderGraphParameterInfo::Type)> &&
                                      TIsTriviallyRelocatable_V<decltype(ShaderGraphParameterInfo::DefaultValue)>;
    };
    ...
    struct ShaderGraphCompileResult
    {
        ...
        FString ErrorLog;
        TArray<ShaderGraphParameterInfo> ExposedParameters;
    };
