...
namespace OloEngine
{
    ...
    // Version 2 invalidated BRDF LUT cache files generated by the old
    // degenerate-quad path (all-zero RG32F LUT → black metallics).
    // Version 3 invalidated SH-irradiance cache files written by the
    // initial SH path, which produced π×-too-bright irradiance because
    // the cosine-lobe coefficients (π, 2π/3, π/4) gave the raw Lambertian
    // integral instead of the *normalised* irradiance the PBR pipeline
    // expects.
    // Version 4 invalidates SH-irradiance cache files written between
    // the π-fix and the IrradianceFromSH.glsl driver workaround: the
...
#pragma pack(pop)
    ...
    Ref<TextureCubemap> IBLCache::LoadCubemapFromCache(const std::filesystem::path& path)
    {
        ...
        {
            OLO_CORE_WARN("IBLCache: Unsupported cache version: {} (expected {})", header.Version, kIBLCacheVersion);
            return nullptr;
        }
    ...
    Ref<Texture2D> IBLCache::LoadTexture2DFromCache(const std::filesystem::path& path)
    {
        ...
        if (IsInvalidBRDFLutPayload(spec.Format, data))
        {
            OLO_CORE_WARN("IBLCache: Rejected all-zero BRDF LUT cache payload: {}", path.string());
            return nullptr;
        }
                                        ...
                                        const std::filesystem::path& path)
    {
        ...
        if (IsInvalidBRDFLutPayload(spec.Format, data))
        {
            OLO_CORE_ERROR("IBLCache: Refusing to cache all-zero BRDF LUT texture: {}", path.string());
            return false;
        }
