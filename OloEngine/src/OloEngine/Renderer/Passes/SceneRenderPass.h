...
namespace OloEngine
{
    ...
    class SceneRenderPass : public CommandBufferRenderPass
    {
      ...
      private:
        ...
        // and the active path is not Deferred. The Deferred channels live in
        // `GBufferDebugPass`, which runs after the late G-Buffer writers
        // (issue #1329); this one stays because on the forward paths ScenePass
        // IS the last writer of the attachment it reads.
