...
namespace OloEngine
{
    ...
    bool CommandSceneRenderPass::SubmitMesh(const Ref<Mesh>& mesh, const glm::mat4& transform, 
                                           const Material& material, CommandAllocator& commandAllocator)
    {
        OLO_PROFILE_FUNCTION();
        
        if (!mesh || !m_Target)
        {
            OLO_CORE_WARN("CommandSceneRenderPass::SubmitMesh: Invalid mesh or target framebuffer");
            return false;
        }
        
        return RenderCommand::SubmitMesh(mesh, transform, material, m_CommandBucket, commandAllocator);
    }
    
    bool CommandSceneRenderPass::SubmitCube(const Ref<Mesh>& cubeMesh, const glm::mat4& transform, 
                                          const Material& material, CommandAllocator& commandAllocator)
    {
        OLO_PROFILE_FUNCTION();
        
        if (!cubeMesh || !m_Target)
        {
            OLO_CORE_WARN("CommandSceneRenderPass::SubmitCube: Invalid cube mesh or target framebuffer");
            return false;
        }
        
        return SubmitMesh(cubeMesh, transform, material, commandAllocator);
    }
    
    bool CommandSceneRenderPass::SubmitQuad(const glm::mat4& transform, const Ref<Texture2D>& texture,
                                          CommandAllocator& commandAllocator)
    {
        ...
        if (!texture || !m_Target)
        {
            OLO_CORE_WARN("CommandSceneRenderPass::SubmitQuad: Invalid texture or target framebuffer");
            return false;
        }
        
        return RenderCommand::SubmitQuad(transform, texture, m_CommandBucket, commandAllocator);
    }
    
    bool CommandSceneRenderPass::SubmitTransparentQuad(const glm::mat4& transform, const Ref<Texture2D>& texture,
                                                    const Ref<Shader>& shader, CommandAllocator& commandAllocator)
    {
        OLO_PROFILE_FUNCTION();
        
        if (!texture || !shader || !m_Target)
        {
            OLO_CORE_WARN("CommandSceneRenderPass::SubmitTransparentQuad: Invalid texture, shader, or target framebuffer");
            return false;
        }
        
        return RenderCommand::SubmitTransparentQuad(transform, texture, shader, m_CommandBucket, commandAllocator);
    }
    
    bool CommandSceneRenderPass::SubmitLightCube(const Ref<Mesh>& cubeMesh, const glm::mat4& transform,
                                               const Ref<Shader>& lightCubeShader, CommandAllocator& commandAllocator)
    {
        OLO_PROFILE_FUNCTION();
        
        if (!cubeMesh || !lightCubeShader || !m_Target)
        {
            OLO_CORE_WARN("CommandSceneRenderPass::SubmitLightCube: Invalid cube mesh, shader, or target framebuffer");
            return false;
        }
        
        return RenderCommand::SubmitLightCube(cubeMesh, transform, lightCubeShader, m_CommandBucket, commandAllocator);
    }
}