...
namespace OloEngine
{
    ...
    //
    // The capture uses cheap `glCopyImageSubData` blits on the GPU, so the
    // CPU never reads pixels back. Each capture texture is allocated once
    ...
    class RenderGraphFrameCapture
    {
      public:
        ...
        enum class Source : u8
        {
            ...
            // RenderGraph blackboard. If these differ from the "live"
            // versions above, the blackboard's physical-slot bookkeeping
            // is corrupt (transient resource planner stomping imported
            // FBs, stale handles, etc.).
            SceneColorViaBlackboard,
            PostProcessColorViaBlackboard,
            COUNT
        };
      ...
      private:
        ...
        // Copy framebuffer color attachment 0 into the per-pass texture.
