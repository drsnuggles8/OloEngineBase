...
namespace OloEngine
{
	...
	public:
		...
		[[nodiscard]] const std::string& GetCurrentStateName() const { return m_CurrentState; }
		[[nodiscard]] bool IsInTransition() const { return m_InTransition; }
		[[nodiscard]] f32 GetCurrentStateNormalizedTime() const;
		[[nodiscard]] const AnimationState* GetState(const std::string& name) const;
		[[nodiscard]] const std::unordered_map<std::string, AnimationState>& GetStates() const { return m_States; }
		[[nodiscard]] const std::vector<AnimationTransition>& GetTransitions() const { return m_Transitions; }
