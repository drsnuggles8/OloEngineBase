...
namespace OloEngine
{
    ...
    // v15: CloudscapeComponent gained the volumetric self-shadow block (issue #723; v14 and older saves omit it and keep the constructor defaults, which have self-shadowing ON — the same look a fresh scene gets). FogSettings' matching fields are deliberately NOT in the archive: the scene-settings section is a flat stream with no length prefix, so it cannot carry a version-gated field — see the note in SaveGameSerializer::SerializeFogSettings.
