...
namespace OloEngine
{
    ...
    static void SerializeLODLevel(FArchive& ar, LODLevel& l)
    {
        ...
        // Error was added in v20 (issue #711). This runs once PER LEVEL inside a
    ...
    {
        // A GENERATED chain is derived data and never goes in a save (issue #711):
        // its levels name memory-only Mesh assets, so the handles are dead in the
        // process that loads them, and restoring them would clobber the chain the
        ...
        ar << c.m_Enabled;
        // v20 (issue #711): whether the chain is generated rather than authored.
        // Read AFTER the levels because that is where a v19 layout leaves off — the
        ...
        {
            autoGenerated = false; // nothing generated a chain in a pre-v20 build
        }
