...
namespace OloEngine::Audio::SoundGraph
{
	...
	class SquareNode : public NodeProcessor
	{
		...
		const Identifier DutyCycle_ID = OLO_IDENTIFIER("DutyCycle");
	...
	public:
		SquareNode()
		{
			...
			AddParameter<f32>(DutyCycle_ID, "DutyCycle", 0.5f); // 50% duty cycle by default
		...
		void Process(f32** inputs, f32** outputs, u32 numSamples) override
		{
			...
			f32 dutyCycle = GetParameterValue<f32>(DutyCycle_ID, 0.5f);

			// Clamp frequency to reasonable range
			frequency = glm::clamp(frequency, 0.1f, static_cast<f32>(m_SampleRate * 0.5));
			// Clamp duty cycle to [0.01, 0.99] to avoid issues
			dutyCycle = glm::clamp(dutyCycle, 0.01f, 0.99f);

			if (outputs && outputs[0])
			{
				for (u32 i = 0; i < numSamples; ++i)
				{
					// Calculate square wave based on phase and duty cycle
					...
					f32 squareValue = (normalizedPhase < static_cast<f64>(dutyCycle)) ? 1.0f : -1.0f;
			...
			else
			{
				...
				f32 squareValue = (normalizedPhase < static_cast<f64>(dutyCycle)) ? 1.0f : -1.0f;
				f32 result = squareValue * amplitude;
		...
		f32 GetDutyCycle() const
		{
			return GetParameterValue<f32>(DutyCycle_ID, 0.5f);
		}
	};
}