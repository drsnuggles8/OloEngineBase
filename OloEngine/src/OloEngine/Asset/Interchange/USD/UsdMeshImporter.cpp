...
namespace OloEngine
{
    ...
    MeshImportResult UsdMeshImporter::Import(const std::filesystem::path& path, const MeshImportOptions& options)
    {
        ...
        // Match the glTF/Alembic UV convention: USD 'st' has a bottom-left origin; flip V so it
        // lands in the engine's top-left space. options.FlipUV inverts when a caller knows better.
        ...
        // If no prim carried a material, hand back one default so consumers resolve something.
        if (materials.empty())
