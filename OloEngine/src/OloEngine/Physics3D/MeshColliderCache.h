...
namespace OloEngine
{
    ...
    class MeshColliderCache
    {
      public:
        ...
        // Settings
        void SetMaxCacheSize(sizet maxSizeBytes)
        {
            m_MaxCacheSize.store(maxSizeBytes, std::memory_order_relaxed);
        }
        sizet GetMaxCacheSize() const
        {
            return m_MaxCacheSize.load(std::memory_order_relaxed);
        }

        void SetMaxConcurrentCooks(u32 maxCooks)
        {
            m_MaxConcurrentCooks.store(maxCooks, std::memory_order_relaxed);
        }
        u32 GetMaxConcurrentCooks() const
        {
            return m_MaxConcurrentCooks.load(std::memory_order_relaxed);
        }
