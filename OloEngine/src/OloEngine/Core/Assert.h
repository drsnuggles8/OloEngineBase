 ...
 */
	...
	#define OLO_CORE_VERIFY_SLOW(expr, ...) do { if(!(expr)) { OLO_CORE_VERIFY_MESSAGE_INTERNAL(__VA_ARGS__); OLO_DEBUGBREAK(); } } while(0)
	#define OLO_VERIFY_SLOW(expr, ...) do { if(!(expr)) { OLO_VERIFY_MESSAGE_INTERNAL(__VA_ARGS__); OLO_DEBUGBREAK(); } } while(0)
#else
 ...
 */
	...
	#define OLO_CORE_CHECK_SLOW(expr, ...) do { } while(0)
	#define OLO_CHECK_SLOW(expr, ...) do { } while(0)
#endif
