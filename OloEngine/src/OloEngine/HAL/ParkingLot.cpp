...
namespace OloEngine::ParkingLot
{
	...
	public:
		...
		explicit operator bool() const { return m_Mutex != nullptr; }
	...
	public:
		static FBucket& FindOrCreateBucket(const void* Address, TDynamicUniqueLock<FWordMutex>& OutLock)
		{
			...
			for (;;)
			{
				...
				FBucket& Bucket = Table.FindOrCreateBucketAtIndex(Index, [] { return new FBucket(); });
	...
	private:
		...
		static bool TryLock(FTable& Table, TArray<FBucket*>& OutBuckets)
		{
			...
			{
				OutBuckets.Add(&Table.FindOrCreateBucketAtIndex(Index, [] { return new FBucket(); }));
			}
