...
TEST(ShaderBindingLayout, TerrainUBOSizeStable)
{
    ...
    // (-> 2256); issue #1336 appended the ambient-ladder controls vec4
    // (-> 2272). The GLSL side is include/TerrainParamsBlock.glsl — ONE
    ...
    EXPECT_EQ(sizeof(UBOStructures::TerrainUBO), 2272u);
}
...
TEST(ShaderBindingLayout, FoliageUBOSizeStable)
{
    ...
    // + six wind/history snapshot vec4s (#1236) = 272, + the two LOD transition
    // lanes (#1237) = 304, + the interaction params lane and its 16-slot
    // influence array (#1238) = 1344.
    ...
    EXPECT_EQ(sizeof(UBOStructures::FoliageUBO), 1344u);
}
