    ...
    namespace
    {
        class CommandLifecycleListener : public ::testing::EmptyTestEventListener
        {
          public:
            ...
            {
                m_CountAtStart = CommandLifecycle::GetTotalViolationCount();
                CommandLifecycle::ResetExpectedViolationsSeen();
            }

            void OnTestEnd(const ::testing::TestInfo& info) override
            {
                const u64 raised = CommandLifecycle::GetTotalViolationCount() - m_CountAtStart;
                if (raised == 0 || CommandLifecycle::ExpectedViolationsSeen())
                    return;

                std::string message = "Command lifecycle: " + std::to_string(raised) +
                                      " violation(s) raised during this test and not declared by a "
