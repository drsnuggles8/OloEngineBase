...
// pulls in) must be listed in the `AllComponents` tuple — and, inversely,
// every entry in that tuple must resolve to a real declared struct.
//
// The tuple is no longer hand-written: OloHeaderTool generates it from the
// `struct *Component` definitions under OloEngine/src into
// `Scene/Generated/AllComponents.Generated.inl`, which Components.h #includes
// at the bottom. This test reads that generated file. It is still a real
// guard, not redundant with codegen: the generator and this test discover
// components differently (the generator scans OloEngine/src recursively; this
// test reads a *curated* header-root list) and each excludes runtime-only
// components via a hand-maintained list, so this test catches drift between
// the two — e.g. a component declared in a header the generator scans but this
// test's `ComponentHeaderRoots()` doesn't yet list (surfaces in the inverse
// test), or an exclusion list that has fallen out of sync.
//
// Why the tuple matters: `AllComponents` is iterated by three separate code
// paths —
...
// other test failure. This guards what was the last unguarded *silent*
// touch-point of the six-point ECS cross-binding checklist in CLAUDE.md. The
// others fail loudly or are covered elsewhere: a missing `OnComponentAdded<T>`
// is a link error, and the SceneSerializer / SaveGame paths each have their
// own coverage test (ComponentSerializerCoverageTest.cpp,
...
// the `kRuntimeOnly` set in ComponentSerializerCoverageTest.cpp AND the
// `kComponentsNotInTuple` set the generator applies in tools/OloHeaderTool.
// =============================================================================
...
namespace OloEngine::Tests
{
    namespace
    {
        ...
        // The `AllComponents` tuple is no longer hand-written in Components.h — it
        // is generated by OloHeaderTool from every `struct *Component` definition
        // (Components.h now just #includes this file at the bottom). This test
        // still guards it in both directions: it reads the generated tuple here
        // and reconciles it against the components declared in the headers below.
        fs::path GeneratedTuplePath()
        {
            return RepoRoot() / "OloEngine" / "src" / "OloEngine" / "Scene" / "Generated" /
                   "AllComponents.Generated.inl";
        }
        ...
        std::vector<fs::path> ComponentHeaderRoots()
        {
            ...
            return {
                ...
                base / "Tilemap" / "TilemapComponent.h",
            };
        }
        ...
        // from the OloHeaderTool-generated tuple file. The tuple is a flat
        // comma-separated list of plain type names (no nested templates), so the
        // first `>;` after `ComponentGroup<` is its terminator.
        std::set<std::string> CollectTupleMembers()
        {
            const std::string headerSrc = ReadFile(GeneratedTuplePath());
    ...
    TEST(ComponentTupleCoverage, EveryDeclaredComponentIsInAllComponentsTuple)
    {
        ...
        ASSERT_FALSE(tuple.empty())
            << "Couldn't parse the `AllComponents = ComponentGroup<...>` tuple in the "
               "generated AllComponents.Generated.inl — regenerate it with "
               "`cmake --build build --target GenerateBindings`, or the generator's "
               "emit shape changed and this test's parser needs updating.";
        ...
        // a dumping ground. This MUST stay in sync with the generator's
        // `kComponentsNotInTuple` set in tools/OloHeaderTool/main.cpp.
        ...
        if (!missing.empty())
        {
            ...
            oss << "\nA component left out of AllComponents is silently skipped by "
                ...
                << "registration (no link error, no other test failure). The tuple is "
                << "now generated by OloHeaderTool, so the usual cause is a STALE "
                << "generated file — run `cmake --build build --target GenerateBindings` "
                << "and re-stage AllComponents.Generated.inl. If the component is "
                << "deliberately runtime-only, exclude it in BOTH the generator's "
                << "`kComponentsNotInTuple` (tools/OloHeaderTool/main.cpp) and this "
                << "test's `kNotInTuple`, with a comment explaining why.\n";
            FAIL() << oss.str();
        }
    }
