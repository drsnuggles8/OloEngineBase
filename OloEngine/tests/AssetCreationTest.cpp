...
TEST_F(AssetCreationTest, ColliderMaterial_Basic)
{
    ...
    // Test default values
    EXPECT_FLOAT_EQ(material.Friction, 0.5f) << "Default friction should be 0.5";
    EXPECT_FLOAT_EQ(material.Restitution, 0.15f) << "Default restitution should be 0.15";
    
    // Test assignment
    material.Friction = 0.8f;
    material.Restitution = 0.2f;
    
    EXPECT_FLOAT_EQ(material.Friction, 0.8f) << "Friction should be assignable";
    EXPECT_FLOAT_EQ(material.Restitution, 0.2f) << "Restitution should be assignable";
}
