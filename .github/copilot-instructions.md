...
**Test architecture (two axes).** Renderer tests are organised into an 11-layer pyramid (property → shader unit → round-trip → GL state → render graph → perf → smoke → golden → cross-vendor → auto-capture → fuzzing) covering rendering contracts. Cross-subsystem behavioural tests live on a separate axis tagged `"Functional"` — they drive a real `Scene::OnUpdateRuntime` to pin seams between Animation × Physics × Scripting × Networking × Audio × Asset × Nav × Save-game × Gameplay × AI. When adding an engine feature, add tests on every axis that applies (decision tree in `.github/instructions/testing-architecture.instructions.md`). Authoritative reference: [docs/testing.md](../docs/testing.md) (both axes, philosophy, anti-patterns, decision tree, per-layer reference, and auto-catalogues in §9.1 / §9.2). The catalogues are generated from `OloEngine/tests/scripts/test_catalogue.json` via `generate_test_catalogue.py`. New `.cpp` files under `OloEngine/tests/{Rendering,ShaderGraph,Streaming,Functional}/` MUST be registered in that JSON — the `test-catalogue-in-sync` pre-commit hook fails otherwise. Never hand-edit the `<!-- BEGIN/END: renderer-catalogue -->` or `<!-- BEGIN/END: functional-catalogue -->` blocks.
