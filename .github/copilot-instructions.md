...
**Test architecture.** Renderer tests are organised into an 11-layer pyramid (property → shader unit → round-trip → GL state → render graph → perf → smoke → golden → cross-vendor → auto-capture → fuzzing). When adding an engine feature, add tests at every layer that applies (see `.github/instructions/testing-architecture.instructions.md` for the decision tree). Authoritative reference: [docs/renderer-testing.md](../docs/renderer-testing.md); the §3.12 catalogue is auto-generated from `OloEngine/tests/scripts/test_catalogue.json` via `generate_test_catalogue.py`. New `.cpp` files under `OloEngine/tests/{Rendering,ShaderGraph,Streaming}/` MUST be registered in that JSON — the `test-catalogue-in-sync` pre-commit hook fails otherwise. Never hand-edit the `<!-- BEGIN/END: auto-catalogue -->` block.
