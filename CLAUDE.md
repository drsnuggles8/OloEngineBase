...
### OloHeaderTool — generated scripting bindings + AllComponents tuple
...
- **Adding a component without updating every hand-maintained touch-point** (`Scene::OnComponentAdded<T>` specialization, `SceneSerializer.cpp`, `SaveGameComponentSerializer.{h,cpp}` **plus the `SAVE_COMPONENT`/`TRY_LOAD_COMPONENT` enumeration lists in `SaveGameSerializer.cpp`**, `LuaScriptGlue.cpp::RegisterAllTypes`, `OLO_PROPERTY` annotations) → scenes don't persist, save-games drop the component, scripts break silently — or, for a missing `OnComponentAdded<T>`, the test/runtime binary fails to link. The pre-commit hook can't catch this; see the Definition of done above. (The `AllComponents` tuple is **no longer** one of these — OloHeaderTool generates it from the component definition automatically; only runtime-only components need the generator + test exclusion edit.)
