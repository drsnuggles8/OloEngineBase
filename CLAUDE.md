...
3. **Cross-binding check** — the pre-commit hook does **not** catch this. If you added or changed an ECS component, you must update the remaining hand-maintained touch-points or scripting / scene saves / save-games / runtime copies will silently drop it (and the build will fail to link):
   ...
   - `Scene/SceneSerializer.cpp` — `SerializeEntity` (writes) and `DeserializeEntityComponents` (reads). Validate every float with `std::isfinite`. **Trivial components are no longer hand-written** (issue #380, fourth slice): OloHeaderTool runs a full data-member scan (`CollectComponentFields` — *not* the `OLO_PROPERTY` scan, since the serializer persists every field, not just script-exposed ones) and, for every `struct *Component` whose **every** data member is a primitive / `glm::vec*` / `std::string`, generates a serialize block + a deserialize block into `Scene/Generated/Scene{Serialize,Deserialize}Components.Generated.inl`, which `SceneSerializer.cpp` `#include`s at the end of `SerializeEntity` / `DeserializeEntityComponents`. So a new **all-trivial** component round-trips through scene YAML with **zero** edits here. You only hand-write a block when the component has a **non-trivial field** (enum, `AssetHandle`, `Ref<T>`, `std::vector`, nested struct, `glm::quat`/`mat`/`ivec`, …) — the parser classifies it non-trivial and skips it automatically — **or** when an all-trivial component needs custom deserialize logic the plain round-trip would drop (range clamps / `Sanitize*` / a runtime-only field the serializer omits / a non-`m_`-stripped key); in that case keep the hand-written block **and** add the component to `kComponentsCustomSerialize` (`tools/OloHeaderTool/main.cpp`) so the generator skips it. Forgetting that exclusion ⇒ the component is emitted **and** hand-written ⇒ a **double-emit** (duplicate YAML key on write, `AddComponent<T>` twice on read) — caught loudly by `ComponentSerializerCoverageTest`'s `NoComponentIsBothHandWrittenAndGenerated` disjointness test, never silent. The same test's existence check now reads `SceneSerializer.cpp` **plus** the two generated `.inl` as one corpus, so a component handled by either path counts as plumbed-through. `kComponentsCustomSerialize` is a **fourth** distinct exclusion set (unrelated to the tuple / save-game / on-add / on-remove sets). The generated deserialize validates floats via `TryReadFiniteF32` (scalars) / the glm `Decode` helpers (vectors); a missing key keeps the constructor default.
