...
- [variable-rate-compute-shading.md](docs/agent-rules/variable-rate-compute-shading.md) — a classifier that coarsens nothing passes every "did it damage the image" test; measure departure from a PLANE, not depth range, and read the heatmap before the statistics.
...
- [compute-written-texture-mip-chain.md](docs/agent-rules/compute-written-texture-mip-chain.md) — every writer of mip 0 owes the chain; SetData rebuilt it as a side effect, so a compute kernel that bypasses the upload path leaves the coarse levels showing the past, and fixing one writer exposes the others.
...
- [terrain-tile-meets-ocean.md](docs/agent-rules/terrain-tile-meets-ocean.md) — a terrain tile ends in a vertical wall where the tile stops, and every shaping knob is uniform so none of them can fix it; measure the OUTERMOST RING, not the frame. §8 is the sting in the tail: the mask that makes the shoreline is itself a ~50 deg slope across the whole outer flank, so an auto-material rock rule keyed below that claims the island and normalizes every other layer to nothing.
...
- [cpu-gpu-surface-parity.md](docs/agent-rules/cpu-gpu-surface-parity.md) — a surface the shader displaces and gameplay samples: a GPU raster cannot be parity-tested so the shared thing must be an analytic record, the two halves can agree on the function and disagree about which SPACE its arguments are in, a probe shader that copies the production GLSL tests the copy, and a physical constant that is a RATIO needs a test that varies the denominator.
- [persistent-world-space-fields.md](docs/agent-rules/persistent-world-space-fields.md) — a decaying accumulation buffer (boat wake, snow melt, damage map): a multiplicative decay is unrepresentable in a normalized-integer texture below a rate threshold, so the field renders perfectly and never fades; a per-frame stamp cannot draw a diverging shape; and `dt` must come from the mockable clock or none of its goldens are stable.
