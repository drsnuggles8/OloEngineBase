...
**That gap is now closed** by [cmake/LinkSemaphore.cmake](cmake/LinkSemaphore.cmake), which routes every link through a wrapper holding one permit of an OS-named semaphore (`OLO_LINK_SEMAPHORE_SLOTS`, default 2 — the measured safe ceiling) that **every build tree on the machine contends for**. A kernel semaphore rather than a lock file, because the kernel reclaims the permit when the holder exits however it exits, so there is no dead-holder case to get wrong. It **fails open**: if the semaphore is unavailable the link runs unthrottled, since a throttle that can fail a build is worse than no throttle.
