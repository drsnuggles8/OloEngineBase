...
allowlist and no exclude list — and classifies every `.cpp` that declares a
`TEST` / `TEST_F` / `TEST_P` / `TYPED_TEST`. Each such file is classified by
**one** of:

- an in-file `// OLO_TEST_LAYER: <id>` comment near the top of the `.cpp`
  (**preferred** — the classification lives in the test file itself, so adding
  a test never touches a shared file and two branches adding tests can't
  collide on it); **or**
- an entry in
  [`test_catalogue.json`](../OloEngine/tests/scripts/test_catalogue.json) →
...
errors if a file carries both (remove the JSON entry once you add a marker).
Valid `<id>` values are the renderer layers `L1`–`L11` / `plumbing` /
`cullinglod` / `shaderpipe` / `integration` / `meta`, plus `Functional` and
`unit`. Pre-commit runs the script in `--check` mode and fails the hook if any
test `.cpp` is classified by neither mechanism, wherever it lives in the tree.
Files with no test macros (the gtest `main`, libFuzzer targets,
fixtures/helpers) are not tests and need no entry.
...
| [OceanFFTVisualEvidenceTest.cpp](../OloEngine/tests/Rendering/PropertyTests/OceanFFTVisualEvidenceTest.cpp) | 4 | **OceanFFTVisualEvidenceTest** &mdash; `CaptureFFTOceanFromMultipleAngles`, `FFTToggleVisiblyChangesSurface`, `GpuComputeToggleLeavesSurfaceUnchanged`, `JonswapSpectrumRendersOpaqueAndDiffersFromPhillips` |
...
**Totals.** 97 renderer-scope test files, 1331 TEST / TEST_F / TEST_P declarations across all layers.
...
| [NavMeshOffMeshLinkBridgesGapTest.cpp](../OloEngine/tests/Functional/Navigation/NavMeshOffMeshLinkBridgesGapTest.cpp) | 1 | **NavMeshOffMeshLinkBridgesGapTest** &mdash; `LinkClosesGapThatIsImpassableWithoutIt` |
...
**Totals.** 92 Functional test files, 162 TEST / TEST_F declarations across all subsystems.
...
| [NavMeshTest.cpp](../OloEngine/tests/NavMeshTest.cpp) | 34 | **NavMeshSettingsTest** &mdash; `DefaultValues`<br/>**NavMeshAssetTest** &mdash; `StaticType`, `DefaultConstructorIsInvalid`, `AssetTypeConsistency`, `SettingsRoundTrip`, `SerializeEmptyMeshFails`, `DeserializeGarbageDataFails`, `MoveConstructor`<br/>**NavMeshQueryTest** &mdash; `DefaultConstructorIsInvalid`, `FindPathWithoutNavMeshReturnsFalse`, `FindNearestPointWithoutNavMeshReturnsFalse`, `RaycastWithoutNavMeshReturnsError`, `IsPointOnNavMeshWithoutNavMeshReturnsFalse`<br/>**CrowdManagerTest** &mdash; `DefaultConstructorIsInvalid`, `AddAgentWithoutNavMeshReturnsNegative`<br/>**NavMeshBoundsComponentTest** &mdash; `DefaultValues`, `CopyPreservesLinks`<br/>**OffMeshLinkTest** &mdash; `DefaultValues`, `EqualityComparesAllFields`<br/>**NavAgentComponentTest** &mdash; `DefaultValues`, `CopyOnlySerializedFields`, `CopyAssignmentResetsRuntime`, `MoveConstructorResetsRuntime`<br/>**AssetTypeTest** &mdash; `NavMeshToString`, `NavMeshFromString`<br/>**NavMeshRoundTripTest** &mdash; `SerializeDeserializePreservesPathfinding`<br/>**NavMeshQueryPositiveTest** &mdash; `FindNearestPointOnPlane`, `RaycastUnobstructed`, `IsPointOnNavMeshCenter`, `IsPointOffNavMeshFarAway`, `CustomQueryBudget`<br/>**CrowdManagerPositiveTest** &mdash; `AddAgentWithValidNavMesh`, `GetAgentPositionAfterAdd`, `RemoveAgentDecrementsCount` |
...
| File | Tests | Cases |
|---|---:|---|
| [CinematicCurveTest.cpp](../OloEngine/tests/Cinematic/CinematicCurveTest.cpp) | 23 | **CinematicCurveTest** &mdash; `EmptyFloatChannelReturnsFallback`, `FloatChannelClampsOutsideRange`, `FloatChannelLinearMidpoint`, `FloatChannelConstantHoldsLeftKey`, `FloatChannelEaseInOutIsSmoothstep`, `ApplyInterpContract`, `BezierBlendEndpointsAreClamped01`, `BezierBlendZeroTangentsIsSmoothstep`, `BezierBlendUnitTangentsIsLinear`, `BezierBlendSteepOutTangentLeadsFaster`, `FloatChannelBezierFlatTangentsMatchEaseInOut`, `FloatChannelBezierUnitTangentsMatchLinear`, `FloatChannelBezierUsesRightKeyInTangent`, `FloatChannelBezierOvershoots`, `Vec3ChannelBezierEasesUniformly`, `QuatChannelBezierStaysFiniteUnitAndClamped`, `Vec3ChannelLinearMidpoint`, `Vec3ChannelDegenerateSegmentIsSafe`, `QuatChannelSlerpMidpointAndNormalization`, `QuatChannelDegenerateKeyDoesNotNaN`, `QuatChannelEmptyReturnsFallback`, `VisibilityTrackStepSemantics`, `VisibilityTrackBeforeFirstKeyUsesFallback` |
...
| [CinematicSerializerTest.cpp](../OloEngine/tests/Cinematic/CinematicSerializerTest.cpp) | 8 | **CinematicSerializerTest** &mdash; `RoundTripPreservesStructureAndValues`, `EmptySequenceRoundTrips`, `MalformedYamlReturnsNullWithoutCrashing`, `NullSequenceSerializesToEmptyString`, `BezierTangentsRoundTrip`, `LegacyFileWithoutTangentsLoadsAsFlatDefaults`, `OutOfRangeInterpClampsButBezierIsAccepted`, `MalformedKeysAreSkippedNotMaterialized` |
...
| [McpStructuredOutputTest.cpp](../OloEngine/tests/MCP/McpStructuredOutputTest.cpp) | 10 | **McpStructuredOutput** &mdash; `StructuredFactoryMirrorsDataIntoTextAndSetsStructuredContent`, `TextAndErrorFactoriesLeaveStructuredContentNull`, `ToolsListEmitsOutputSchemaWhenPresent`, `ToolsListOmitsOutputSchemaForTextTool`, `ToolsListOmitsOutputSchemaWhenEmptyObject`, `ToolsCallReturnsStructuredContentAndTextMirror`, `ToolsCallOmitsStructuredContentForTextTool`, `ToolsCallErrorHasNoStructuredContent`, `RedactionScrubsPathsInStructuredContent`, `RedactionOffLeavesStructuredContentIntact` |
...
| File | Tests | Cases |
|---|---:|---|
| [TerrainGeneratorTest.cpp](../OloEngine/tests/Terrain/TerrainGeneratorTest.cpp) | 20 | **TerrainGeneratorTest** &mdash; `HeightFieldIsDeterministic`, `HeightFieldIsNormalizedAndFinite`, `LargeSeedStillProducesVariedTerrain`, `DifferentSeedsProduceDifferentTerrain`, `RidgedAndWarpAndExponentStayValid`, `TerraceShapingStaysValidAndDeterministic`, `ErosionPostPassIsDeterministic`, `ErosionStaysNormalizedAndFinite`, `ErosionActuallyChangesTheField`, `ApplyErosionStandaloneIsDeterministicAndGuarded`, `TerraceEndpointsAndIdentity`, `TerraceIsMonotonicAndBounded`, `TerraceProducesFlatPlateaus`, `RuleWeightPeaksInsideBandAndZeroOutside`, `SlopeBandSelectsRule`, `DefaultRulesAssignExpectedLayers`, `LayerWeightsAreNormalized`, `NoMatchingRuleFallsBackToLayerZero`, `PackLayerWeightsQuantizesToBothSplatmaps`, `DISABLED_DumpErosionHeightmapPNGs` |

**Totals.** 151 unit / subsystem test files, 1877 TEST / TEST_F declarations across all subsystems.
