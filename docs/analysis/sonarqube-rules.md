...
(`6.82.1-SNAPSHOT`) doesn't match the analyzer version (`6.82.1.100194`).

The premise of the original deferral above is now **stale**: `SonarCloud.yml`'s build step
has since moved to `cmake -S . -B build -G "Ninja Multi-Config" ... -DCMAKE_EXPORT_COMPILE_COMMANDS=ON`
(with `cl.exe` on `PATH` via `msvc-dev-cmd`), so a CMake-native `compile_commands.json` *would*
be produced now — the "naive swap craters the C++ issue count" failure mode described above no
longer applies as written. That said, we did **not** need to go that route:

- Recent `build-wrapper` releases (the one `sonarqube-scan-action@v8.2.0`'s
  `install-build-wrapper` step installs) emit a standard `compile_commands.json` **inside their
  own `--out-dir`**, alongside the legacy dump — this is exactly what the deprecation message's
  own follow-up sentence says ("switch to `sonar.cfamily.compile-commands` **after using the
  matching version of build-wrapper**").
- So the fix is a one-line change to the SonarQube Scan step's `args`: swap
  `--define sonar.cfamily.build-wrapper-output=<dir>` for
  `--define sonar.cfamily.compile-commands=<dir>/compile_commands.json`. The `Install Build
  Wrapper` / `Run build-wrapper` steps are **unchanged** — build-wrapper still wraps the whole
  build and still captures every subprocess (including the `OloHeaderTool` codegen custom
  target), it just also writes this file now.
- This sidesteps the CMake-generator question entirely (no dependency on Ninja vs. MSBuild,
  no risk of an empty/duplicate-config compile_commands.json) and drops the deprecated
  `build-wrapper-output` property.

Shipped in `.github/workflows/SonarCloud.yml` and validated via a `workflow_dispatch` run
(29009983658, 2026-07-09) on this branch:

- `CFamily analysis configuration mode: Compile-Commands` confirms the analyzer used the new
  ingestion path, not the deprecated one.
- The `sonar.cfamily.build-wrapper-output` **deprecation warning is gone** (it only fires when
  that property is set).
- 536 compilation units analyzed, 22,433 issues raised, `EXECUTION SUCCESS` — a normal-sized
  analysis, not the "near-empty" failure mode the old deferred note worried about.
- **The build-wrapper/analyzer version-mismatch warning is still present** ("was generated
  using build-wrapper version 6.82.1-SNAPSHOT, which does not match the analyzer version
  6.82.1.100194"). This turned out to be **independent of which sonar property is used**: the
  `compile_commands.json` build-wrapper emits carries its own embedded provenance ("Compilation
  database was generated by build-wrapper 6.82.1-SNAPSHOT"), and the analyzer checks that
  regardless of the `build-wrapper-output`/`compile-commands` ingestion path. The mismatch is
  between what `sonarqube-scan-action@v8.2.0`'s `install-build-wrapper` step downloads (a
  `-SNAPSHOT` build) and the `CFamily plugin version: 6.82.1.100194` the same action's scan step
  bundles — an upstream inconsistency in that action's own release, not something fixable from
  this repo's CI config short of manually side-fetching a different build-wrapper build (not
  worth the fragility). It's a `WARN`, not a failure, and doesn't affect analysis correctness —
  left open, revisit if a future `sonarqube-scan-action` release resolves the skew.

---

## 11. `cpp:S990`, `cpp:S963`/`S958`/`S960`/`M23_042`, `cpp:S1820`, `cpp:S5421` — the debug-lever registry (#714/#818)

**Status:** ✅ **APPLIED** — `env_s990`, `lever_s963`, `lever_s958`, `lever_s960`,
`lever_m23042`, `lever_s1820`, `interactivity_s5421` in `sonar-project.properties`.

### `cpp:S990` — Environment.cpp's one `getenv`

Before this, `Core/Environment.cpp` carried an inline `// NOSONAR cpp:S990` comment as its
justification. It never worked: SonarQube's `NOSONAR` suppression must be a trailing comment
**on the flagged line itself**, and this one sat several lines above the actual `std::getenv`
call as prose inside a larger explanatory comment block — syntactically just a comment, not a
directive the analyzer recognises. The rule kept firing, and PR #818's Reliability Rating on
New Code stayed D against every subsequent push until this exclusion was added. Lesson for the
next one-legitimate-`getenv` site: this file is the pattern to copy, not the old comment.

### `cpp:S963` / `S958` / `S960` / `M23_042` — the `DebugLevers.inl` X-macro table

`Core/DebugLevers.inl` is included several times from `DebugLevers.h` and `DebugLevers.cpp`
with different `OLO_LEVER_TOGGLE`/`EXACT`/`TRISTATE`/`INT`/`NUMBER`/`TEXT` macro definitions —
the same generation trick `Scene.cpp` uses for `AllComponents`/`OnComponentAdded` (see
CLAUDE.md), applied to a table of ~21 debug/diagnostic levers instead of ECS components. Four
rules fire on the `#define`/`#undef` pairs themselves:

- **S963** "parenthesise macro parameters" — the existing false-positive class documented in
  §4 above, for the same reason: these aren't ordinary function-like macros misusing their
  parameters, they're a deliberate codegen mechanism.
- **S960**/**M23_042** "replace with a C++ function" — can't: the six macro shapes each expand
  to something a single function signature cannot produce (a struct field, a setter
  definition, a seed statement, a `Snapshot()` row). That variety is the entire reason to use
  X-macros instead of hand-writing 21 near-identical blocks six times over.
- **S958** "move `#define`/`#undef` to the global namespace" — deliberately the opposite: the
  macros are scoped to the one local block that generates from them and `#undef`'d immediately
  after, specifically so `OLO_LEVER_TOGGLE` etc. never leak into any other TU's macro namespace.

### `cpp:S1820` — `DebugLevers.cpp`'s `Storage` struct, one field per lever

21 fields (at the time of writing), each generated by the same X-macro table above. The field
count IS the lever count by construction — there is no sub-grouping that would reduce it
without breaking the mechanical 1:1 mapping between "a lever exists" and "a field exists",
which is what lets the accessors/setters/seed logic all be generated instead of hand-written.
The sibling `Overridden` struct has 20 fields (one per lever except the read-only `TEXT` kind,
which has no override flag) and sits at the threshold rather than over it, so it needs no
exclusion of its own.

### `cpp:S5421` — `Interactivity.cpp`'s `s_NonInteractive`

The pre-existing false-positive class from `sonar-cleanup-deferred-rules.md` (a mutable
`std::atomic` ref-count/flag misread as "should be const"): `SetNonInteractive()` exists
specifically to flip this at runtime, so making it `const` would delete the function it backs.
...
supports C#, Java, JavaScript, Python and TypeScript; C++ is not in that list (Python and
C# are the two languages currently "coming soon" beyond the original four, and C++ support
is listed as merely "under consideration"). Since `generate_test_catalogue.py` and
`perf_trend.py` are the only Python files in the repo, they're also the only files the
architecture graph can parse — this matches the issue's own report exactly, and the two
files it names are not a symptom of a misconfiguration, they're the entire indexed set for
a supported language. No `sonar-project.properties` change can fix this; revisit if/when
SonarCloud ships C++ architecture support upstream.
