...
- This sidesteps the CMake-generator question entirely (no dependency on Ninja vs. MSBuild,
  ...
  version-gated `build-wrapper-output` ingestion path — which is also almost certainly the
  source of the version-mismatch warning itself.

Shipped in `.github/workflows/SonarCloud.yml`. Verify on the next scan that both the
deprecation warning and the version-mismatch warning are gone.
