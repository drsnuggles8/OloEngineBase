...
| File | Tests | Cases |
|---|---:|---|
| [RenderGraphPathSwitchTests.cpp](../OloEngine/tests/Rendering/RenderGraphPathSwitchTests.cpp) | 5 | **RenderGraphPathSwitch** &mdash; `ForwardToDeferredInsertsDeferredPassesAndEdges`, `DeferredToForwardRemovesDeferredOnlyPasses`, `ForwardToDeferredMissingLightingEdgeIsFlagged`, `AlternatingRebuildsHaveStablePassCounts`, `PassOrderReflectsCurrentTopologyAfterSwitch` |
...
**Totals.** 51 rendering-scope test files, 740 TEST / TEST_F / TEST_P declarations across all layers. Non-rendering subsystems (audio, networking, gameplay, save-game, &hellip;) are catalogued separately and are not listed here by design.
