...
- Production declarations now live in `Setup(RGBuilder&, FrameBlackboard&)`
  across the runtime pass layer; legacy/static declaration coverage survives
  only through explicit `RenderGraphNode` overrides or test-only helpers.
...
**How it works.** Production nodes declare reads and writes during
`Setup(RGBuilder&, FrameBlackboard&)`, and the built frame validates hazards
from those authoritative per-frame access declarations. Explicit node-level
legacy declarations still exist for topology-time diagnostics and focused test
fixtures, so `RenderGraph::ValidateResourceHazards()` can flag wiring problems
before build while `ValidateCompiledResourceHazards()` remains the
authoritative per-frame check for graph-native runtime passes.
...
**Totals.** 51 rendering-scope test files, 1041 TEST / TEST_F / TEST_P declarations across all layers. Non-rendering subsystems (audio, networking, gameplay, save-game, &hellip;) are catalogued separately and are not listed here by design.
