...
- **Self-inflicted error** — a test that deliberately provokes a GL error (e.g. `GLStateGuardTest.RestorePolicy_RestoresCoreStateOnDtor` binds an intentionally-unlinked program → `GL_INVALID_OPERATION`) must drain it before returning. Call `Utils::DrainGLErrors()` (`Platform/OpenGL/OpenGLUtilities.h`, a bounded drain) at the end of the test.
