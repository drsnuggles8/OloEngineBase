...
- **GL-context tests** (`RendererAttachedTest` subclasses; the `*VisualEvidence*` / `*Bake*` / GPU-contract families) **SKIP** on CI (no GL 4.6 context), so they are no-ops under `-j` there. Locally they *run* and contend for the single GPU — expect them to be slow or to hit the per-test `--timeout` at high `-j`; that is a local artifact, **not** a CI failure. Verify GPU tests at a low `-j`. **When a GL-context test fails on your branch, run the same filter against a throwaway `origin/master` worktree before debugging your change.** CI never runs these tests, so two PRs merged from parallel branches can break one without anyone seeing it: the three hand-driven `GPUReadbackStatsEvidence` cull tests failed on `master` from the observer-camera merge (2026-08-21) until #1011 (2026-09-02), because the culler had switched to `Renderer3D::GetCullViewProjectionRelative()` while the tests still uploaded their own camera UBO. Identical assertion lines and values on `master` mean a pre-existing break: fix it in its own commit and say so in the PR body.
...
**No PR job runs the DEVICE-BACKED half of this on HARDWARE, and that is a fact about the hardware,
not an omission.** (The device-free `VulkanCoverageReportTest` does run there, like any other test.)
The AMD self-hosted runner cannot run Vulkan at all (RADV has no descriptor heap) and the hosted
runners have no GPU, so an ordinary PR run says `NOT EXERCISED` — and the banner is what keeps that
honest.
...
What lavapipe can and cannot do for this engine — including the Mesa 26.2.0 floor, which is set by
the ADR 0010 extension contract rather than by the API version — is
[vulkan-software-driver-ci.md](vulkan-software-driver-ci.md). Hardware Vulkan remains a
developer-box capability.

It is **not the ordinary PR gate**: nightly plus `workflow_dispatch` means "caught within a day",
not "blocks review", and a renderer PR pays nothing for it. It does also run on a `pull_request`
that touches one of two paths — the workflow file itself, or `VulkanCapabilities.*` — because those
are the two changes that can silently turn a green nightly into a job verifying nothing, and that
anti-rot arm is the one place a PR run CAN report `EXERCISED`. Same trade `vulkan-off.yml` and
`steam-stub.yml` make for their filters.
