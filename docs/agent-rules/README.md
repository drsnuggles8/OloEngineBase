...
- [groom-into-the-shadow-techniques.md](groom-into-the-shadow-techniques.md): a caster family whose geometry is built after the shadow pass needs the geometry lifted, not a new list; widen a strand to one shadow texel and accept an OPAQUE cast, because a depth target has no alpha and a hashed discard would have nothing to converge it; sample the scene shadow at the coat’s light-exit point so a coat that is also a caster is not occluded by itself twice.
...
- [diagnostic-views-read-the-final-resource-version.md](diagnostic-views-read-the-final-resource-version.md): a debug extraction of a resource with several writers is its own graph node after the last of them, and it records which content version it read.
...
| [diagnostic-views-read-the-final-resource-version.md](diagnostic-views-read-the-final-resource-version.md) | The G-Buffer debug view was blitted at the end of its FIRST writer, so it showed a surface three later passes went on to change; a missing decal and a decal written after the capture are the same picture. |
| [diagnostic-views-read-the-final-resource-version.md §2](diagnostic-views-read-the-final-resource-version.md#and-it-has-to-reach-the-screen-on-every-backend) | `glBlitFramebuffer` converts formats and `vkCmdCopyImage` does not, so the RGBA8 debug channels were a warn-once no-op on Vulkan and the viewport kept its clear — a diagnostic that renders nothing looks exactly like a renderer that drew nothing. |
...
| [groom-into-the-shadow-techniques.md](groom-into-the-shadow-techniques.md) | A whole coat casts nothing while every counter reads healthy, because its strands are a thousandth of a shadow texel wide; a coat that is both caster and receiver is darkened twice, once in the cascade map and once in its own density volume; a second directional technique silently drops the family nobody routed into it; a Vulkan y-flip makes a signed projection scale negative and removes the coat from every shadow map with nothing logged. |
