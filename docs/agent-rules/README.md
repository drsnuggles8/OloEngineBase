...
- [renderer-pr-gpu-tests-run-after-merge.md](renderer-pr-gpu-tests-run-after-merge.md): PR CI has no GPU, so run the evidence, CrossPath and Vulkan device suites a renderer change reaches before the PR; when a fix makes a neutral input read correctly, also rerun every test that measures the feature, because some may be calibrated on the defect.
...
- [renderer-state-machine-harness.md](renderer-state-machine-harness.md): a new renderer cache, lever or execution mode joins the state-machine manifest as an operation or an equivalence pair, with a negative control the harness demonstrably catches; generated sequences, not off/on tests, find the order that breaks it.
...
- [groom-deformed-coat-self-shadowing.md](groom-deformed-coat-self-shadowing.md): bake a bound coat from the strands the pass draws, in groom object space, rebake it when they drift past half a voxel, and refuse to sample a volume more than two voxels stale.
...
- [groom-card-coverage.md](groom-card-coverage.md): cook a card as wide as its members cover when drawn at the hand-over, never their sum; bake a card-tier self-shadow at the coat's fibre area; compensate the strand budget per role; measure a LOD on a frozen pose with a linear metric.
- [groom-coat-bake-cost.md](groom-coat-bake-cost.md): split a bake's time by stage before cutting it; bake a walking coat from a fixed, hashed subset taken before the pose is evaluated, sized from its own occupied voxels; rewrite a volume in place only when no unsubmitted recording can read it.
- [groom-gpu-strand-deformation.md](groom-gpu-strand-deformation.md): build a bound coat's stream once in each root's bind frame and move it in the vertex shader from a per-frame buffer of root transforms and guide displacements; keep the CPU rebuild as a lever-selected reference held to bit-exact agreement; hold an integer in a float lane as a value, never as bits.
...
- [alpha-cutout-mip-chains.md](alpha-cutout-mip-chains.md): build an alpha cutout's mips by matching each level's alpha histogram to level 0's, which is right at every cutoff at once; stop at 64 texels, and correct the chain against what BC7 decodes to.
- [vulkan-block-compressed-upload.md](vulkan-block-compressed-upload.md): upload a cooked BCn chain level by level with whole-level texel extents, carry BC4's (R,R,R,1) swizzle on every sampled view description, and refuse an unsupported format by name with an unloaded texture.
...
- [framebuffer-bind-owns-the-viewport.md](framebuffer-bind-owns-the-viewport.md): `Framebuffer::Bind()` sets the viewport to the target's active size on every backend, and a pass that sets its own takes it from `GetActiveViewportWidth/Height()`, never the spec.
- [oit-blend-state-and-identity-lanes.md](oit-blend-state-and-identity-lanes.md): a pass drawing into the WB-OIT targets re-states its per-attachment blend after anything that can change it; an identity lane is decoded from the texel, never a filtered fetch.
...
- [shared-uniform-binding-bind-before-dispatch.md](shared-uniform-binding-bind-before-dispatch.md): call `Bind()` on a uniform buffer before every dispatch that reads it when its binding point is shared; `SetData` writes the buffer, but the shader reads whichever buffer bound that point last.
...
- [a-pass-opens-its-own-colour-mask.md](a-pass-opens-its-own-colour-mask.md): a pass sets the colour write mask of every attachment it writes, right before its draw, and a pass that narrows the mask or replays packets widens it and invalidates the render-state cache before it returns; selecting the draw buffers does not open them.
...
- [render-graph-repopulate-forces-setup.md](render-graph-repopulate-forces-setup.md): when the blackboard re-populates, invalidate the build cache before `BuildFrameGraph`; invalidating it from inside `PopulateBlackboard` is re-armed by the same frame's build.
...
- [screen-uv-unpack-takes-the-row-flip.md](screen-uv-unpack-takes-the-row-flip.md): a pass that unpacks screen uv into view space builds its constants from `AdjustProjectionForShaderReconstruction`, and a screen direction takes its view signs from that unpack.
...
- [water-displaced-once-and-pinned-captures.md](water-displaced-once-and-pinned-captures.md): if a program has a tess-eval stage the vertex stage must not displace, and an A/B of animated content needs a pinned clock.
...
- [per-draw-material-stamp-rides-the-copy.md](per-draw-material-stamp-rides-the-copy.md): a value stamped onto a per-draw `Material` copy must be in the hand-written copy constructor and `operator=`, and a test of it must go through the path that makes the copy.
...
| [screen-uv-unpack-takes-the-row-flip.md](screen-uv-unpack-takes-the-row-flip.md) | GTAO and sphere-proxy AO reconstructed every Vulkan pixel mirrored about the horizontal on master; a symmetric horizon seed hid it until the XeGTAO port made a 45-degree floor read 0.216 on Vulkan and 0.996 on GL. |
...
| [renderer-state-machine-harness.md](renderer-state-machine-harness.md) | Four renderer bugs (#530, #563, #1315, #1397) each lived in a sequence of valid operations, and each had an off/on test that passed because it started from a clean state. A skipped binding reset is worse: it breaks every frame alike, so even a sequence-versus-fresh comparison stays green. |
...
| [oit-blend-state-and-identity-lanes.md](oit-blend-state-and-identity-lanes.md) | A transparent decal drew, its fragments survived and the composite was unchanged; two identical skin profiles in different slots rendered 3/255 apart. |
...
| [cross-path-term-measurement-traps.md](cross-path-term-measurement-traps.md) | Every "direct light is invariant under GTAO" check passed while GTAO never ran, and ReSTIR DI matched the raster loop bit for bit because it had stood down; read after the frame, a 7.6 lighting term came back as 0.76 from memory a later pass had reused. |
| [render-graph-repopulate-forces-setup.md](render-graph-repopulate-forces-setup.md) | A cached graph ran on a freshly re-populated blackboard for one frame per upscale toggle in Deferred + SSR. The verify lever hides it by construction, and the headless upscale tests run with SSR off. |
...
| [alpha-cutout-mip-chains.md](alpha-cutout-mip-chains.md) | A box mip chain keeps a cutout's coverage at cutoff 0.5 and loses it at every other cutoff (+47% at 0.25, −83% at 0.75 on the pine card), so a control that checks only 0.5 passes on the chain that thins. |
| [vulkan-block-compressed-upload.md](vulkan-block-compressed-upload.md) | Every cooked texture was null on Vulkan and the scene still rendered, with the missing-texture fallback in its place. |
...
| [shared-uniform-binding-bind-before-dispatch.md](shared-uniform-binding-bind-before-dispatch.md) | The vegetation deformer wrote its params with `SetData` and never bound them. It worked until an animated fox's deformer bound its own block at the shared slot one pass earlier; then the vegetation shader read the fox's 48 bytes as its own 96 and device-faulted on Vulkan Deferred. A reduced scene without the fox completed, so the fault looked like a property of the animal path. |
...
| [a-pass-opens-its-own-colour-mask.md](a-pass-opens-its-own-colour-mask.md) | A debug line ending the G-Buffer bucket left attachments 1–4 masked in the editor only: no deferred skin diffusion or snow blur, every Forward OIT frame black, no error, a texture at its clear value. The forward prepass likewise left SSAO drawing into an all-false mask. |
...
| [per-draw-material-stamp-rides-the-copy.md](per-draw-material-stamp-rides-the-copy.md) | The expression-driven pore band (#1243) was inert on every path from the day it shipped: the stamp returns its copy in a `std::optional`, `Material`'s hand-written copy constructor dropped the field, and the only test hand-stamped the component's material, which the rigid path draws without copying. |
...
| [lighting-signal-contract.md](lighting-signal-contract.md) | The ambient ladder (forward, deferred, and the rung SSGI replaces), one `E -> E/pi` conversion, and the composition every raster path calls — each once, where there had been copies. |
...
| [groom-deformed-coat-self-shadowing.md](groom-deformed-coat-self-shadowing.md) | A walking animal carrying its bind-pose shadow; a pose bake judged against another volume, which measures only the two quantisations; a negative control displaced along the light, where the wrong answer is right by accident; a cadence justified by a worst-error claim the numbers refute. |
...
| [groom-card-coverage.md](groom-card-coverage.md) | A card tier drawing 2.6x its coat, to card widths summed over members that overlap; the same tier reading 15% light, to a self-shadow baked from the cards' drawn width instead of the coat's fibre; guard hair 2.2x thick one rung out, to one width compensation for roles thinned at different strides; a LOD error measured at 1.35 that was 1.9, to a walk that advanced between the coat-on and coat-off captures. |
| [groom-coat-bake-cost.md](groom-coat-bake-cost.md) | 95 ms a frame for three walking coats' self-shadow, cut to 11.5, where the stage the issue suspected (the texture upload) was 2% of it; a subset taken inside the bake, which would have left the pose evaluation untouched. |
| [groom-gpu-strand-deformation.md](groom-gpu-strand-deformation.md) | A herd's frame, to every bound strand rebuilt and re-uploaded on the CPU each frame (hundreds of MB, ~90% of the pass's GPU time); every coat on Vulkan, to a per-frame payload snapshotted into a 16 MiB frame arena; every strand on root 0, to a small integer bit-cast into a float vertex lane that a fetch may flush as a denormal. |
