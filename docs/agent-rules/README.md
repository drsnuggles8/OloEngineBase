...
- [oit-and-colour-mask-ambient-state.md](oit-and-colour-mask-ambient-state.md): a pass drawing into the WB-OIT targets re-states its write masks and per-attachment blend after anything that can change them; a pass that replays packets ends with a global `SetColorMask(true, ...)`; an identity lane is decoded from the texel, never a filtered fetch.
...
- [a-pass-opens-its-own-colour-mask.md](a-pass-opens-its-own-colour-mask.md): a pass sets the colour write mask of every attachment it writes, right before its draw, and restores any mask it narrows; selecting the draw buffers does not open them.
...
| [oit-and-colour-mask-ambient-state.md](oit-and-colour-mask-ambient-state.md) | A transparent decal drew, its fragments survived and the composite was unchanged; a leftover write mask blacked out every Forward OIT frame and silenced Deferred skin diffusion in the editor only; two identical skin profiles in different slots rendered 3/255 apart. |
...
| [a-pass-opens-its-own-colour-mask.md](a-pass-opens-its-own-colour-mask.md) | A debug line ending the G-Buffer bucket left attachments 1–4 masked, so DeferredLighting's hand-off output reached nothing: no deferred skin diffusion or snow blur, no error, a texture at its clear value. The forward prepass likewise left SSAO drawing into an all-false mask. |
