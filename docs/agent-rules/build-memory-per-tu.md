...
| `-gsplit-dwarf` (**not** `--gdb-index`) | **Available behind `OLO_SPLIT_DWARF`, default OFF, and the trade is measured.** ccache caches and restores the `.dwo` correctly (cold compile produced `.o`+`.dwo`; with both deleted a cache HIT restored both at identical sizes). The cost is inlined frames: this repo maps `DW_AT_comp_dir` to `/olo`, and split DWARF resolves its side-car through that field, so the symbolizer looks for `/olo/<name>.dwo`. Measured with llvm-symbolizer, one control per variable — `-gsplit-dwarf` alone: inline frames **resolve**; `-ffile-prefix-map=/olo` alone: **resolve**; both together: **lost**. Function names (`.symtab`) and file:line (`.debug_line`) survive in every arm, and every sanitizer job is a Debug build where there is little inlining to lose. A `.dwp` did not restore it in that probe, but the probe symbolized a `.o` rather than an executable, so treat that route as unproven rather than ruled out. `--gdb-index` is deliberately NOT paired: lld builds that index by reading the debug info this flag exists to skip, so it works against the memory goal. `build-memory.yml`'s `Debug + ASan + split DWARF` cell prices the saving on the schedule. |
