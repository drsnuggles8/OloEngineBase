...
- **`f4fef24b` — star-field hash made bit-exact across vendors.** The old `fract(sin(dot(p,k))*43758.5)`
...
## Why NOT "just add a CPU contract test that goes red at the same time"

Tempting, and the issue asked to consider it — but it does **not** fit here, and saying why is the point:
...
The durable guard is the **same-PR rebake discipline made visible at the change site**, not another
normalisable red.
