...
3. **A key that embeds a content hash needs the hash stripped when you group entries for
   ...
   immortal — and so is the entry it superseded. **And the strip has to be tested against
   a real key, not against the example in the comment beside it.** #1205 added a
   `-<ImageVersion>` runner-image stamp to the vcpkg keys and a strip to match, written
   against a two-part example (`20260828.587`); a real `ImageVersion` has three parts
   (`20260907.229.1`), so the strip never fired, the hash strip behind it never fired
   either, and 1083 MiB sat stranded across two lineages until it was measured on
   2026-09-13. The check is one line — run the workflow's own `sed` over the live key
   list and count the groups.
...
Re-measured 2026-09-13 (#1084), after #1170, #1205, #1207, #1208, #1209 and #1204.
The 2026-09-09 figures are kept in each row so the movement is visible. Sizes are the
**compressed tarball**, which is what
counts against the cap — not `SCCACHE_CACHE_SIZE`, which bounds the local directory
before compression:

| Entry | Size |
|---|---|
| `sccache-asan-lsan-linux-llvm-23.1.0` / `-ubsan-` / `-tsan-` | **1114 + 1191 + 1091 = 3396 MiB measured** 2026-09-13 (3440 MiB on 2026-09-09; the lineage is re-banked by the nightly, so this is drift within one cap, not a change). Untouched by #1207/#1208, which changed how the *tests* are scheduled, not what is compiled. **Rule 5 applies to this row and is worth re-reading: these three entries serve the NIGHTLY and fork PRs only.** Same-repo PR sanitizers route to the box and its local ccache, which #1209 raised to 120G and shared across runner slots — local disk, no save step, no cap, nothing on this table. 3396 MiB of a 20000 MiB ceiling is the price of the nightly's build half. First measured 2026-09-09, the first full-size banking at the **1300M** cap #1118 raised them to (from 900M, which measured under all three object sets — `du` put those at 945 / 994 / 1084 MiB and returned 93.88 / 69.89 / 30.62 % hit rates). They land at 86–91 % of the cap, as rule 6 predicts. The 2231 MiB this row carried on 2026-09-08 was the 900M-era set and is not comparable. Both halves of the cap decision: [sccache-cap-vs-object-set.md](sccache-cap-vs-object-set.md). Removed entirely 2026-09-06 by #1082 at 0.00 % (see *A cache can be worth removing*), re-added by #1095 once the compiler underneath the key stopped rolling. |
| `vcpkg-Windows-x64-windows-static-md` | **1448 + 756 = 2204 MiB measured** 2026-09-13, **two live entries**, under #1205's new key shape — up from **706 MiB** on 2026-09-09 (692 on 2026-09-06), and the doubling is **not measured**: #1205 started a fresh lineage keyed on the runner image as well as the manifest, and a fresh lineage is normally *smaller* (see the ASan Windows row). State it as an observation, not a cause. The **second entry, 756 MiB in the pre-#1205 spelling, was stranded** — the prefix strip meant to collapse the two did not fire; see rule 3 and the fix below. Counted here because it is charged to the cap like any other entry. |
| `vcpkg-Linux-x64-linux` | **327 + 327 = 654 MiB measured** 2026-09-13 — the per-entry size is unchanged from 2026-09-09, but there are **two live entries**, for the same reason as the Windows row: #1205's image stamp started a new lineage and the predecessor was stranded. |
| `Linux-vulkan-prebuilt-sdk-true-1.4.357.0` | **314 MiB measured** 2026-09-13. The 2026-09-09 row said "no 1.4.357.0 Linux entry exists" and listed `-1.4.321.0` at 291 MiB; the weekly `video-ffmpeg.yml` linux arm has since banked the stamped one and the old one aged out. One entry, not two. |
| `Windows-vulkan-prebuilt-sdk-true-1.4.357.0` | **256 MiB measured** 2026-09-13 — **one entry now.** The `-1.4.321.0` sibling reached its 30-day age-out and is gone, which is the slow path this row was written to warn about. It cost 229 MiB for about five weeks. The 2026-09-09 state was **256 + 229 MiB — one dependency, two live entries.** This is the version-in-the-key case rule 6's bullet list warns about: `cache-prune.yml` strips a trailing *hex* hash, and `1.4.321.0` is not hex, so the bump superseded nothing and the old entry sits until the 30-day age-out collects it (created 2026-07-07, last read 2026-09-08). 485 MiB for one SDK is the measured price of that key shape. |
| `ffmpeg-Windows-097d85ba…` | **4 MiB, and live** — measured 2026-09-13 restoring on a PR run (`Cache restored from key: ffmpeg-Windows-097d85ba…`, run 34757709174). #1141's fix is confirmed working fleet-wide, and the dead `-n7.1-637be53f…` lineage described next has aged out. Previously: `ffmpeg-Windows-n7.1-637be53f…`, **4 MiB, and dead.** Its hash no longer matches any workflow, so nothing will ever restore it; the age-out collects it. The FFmpeg cache had in fact **never been saved on any run** — the path cached was not the path CMake installs into, so every save exited zero on a `Path Validation Error` (#1141). The working lineage that replaces it is keyed `ffmpeg-<os>-<hash of build-ffmpeg.sh + cmake/ffmpeg.cmake>` and banks ~19 MB uncompressed. |
| `olo-shader-toolchain-Linux-X64-v2026.3-vulkan-sdk-1.4.357.0-v1` | **10 MiB measured** 2026-09-13. New since the last revision of this table (#1139). Carries no run id and no hash, so `cache-prune.yml` leaves it alone by design — it is superseded by editing the `-v1` suffix, the same shape as the Vulkan SDK rows, with the same caveat that a bump strands the predecessor for thirty days. At 10 MiB that is not worth engineering around. |
| **measured subtotal** | **3442 MiB** (4 + 10 + 256 + 314 + 654 + 2204), measured 2026-09-13. Was **1813 MiB** (4 + 229 + 256 + 291 + 327 + 706) on 2026-09-09. Of the +1629, **+1083 is the two stranded vcpkg predecessors** that the prefix-strip fix collects and **+742 is the unexplained vcpkg-Windows growth**; the SDK rows went *down* 227 MiB as the 1.4.321.0 Windows entry aged out. |
| `sccache-windows-2025-release-vk1.4.357.0` | **2046 MiB measured** 2026-09-13, under #1170's stamped spelling, re-banked by every merge. **The "expect it to measure under 2044 MiB at first" prediction below did not hold**, and the reason is worth keeping: `sccache-asan-windows-2025` shrank on rename because its old entry had accumulated across many source generations, whereas this one was *already* at its 2 G cap and evicting, so a fresh lineage refilled to the same cap within a run or two. A rename shrinks a lineage only if the old one was an accumulation; a capped one comes straight back. It is still **at the cap and evicting**, and its cap is still the one open cap question on this table. Previously **2044 MiB** — measured 2026-09-08 and again 2026-09-09 under the key's old, unstamped spelling. At its 2 G cap and evicting. Warm PR runs return 94.77 / 95.01 / 98.49 %, so the cap is not currently costing much, but it has no headroom left either. **#1084 put the Vulkan SDK version in the key**, mirroring what #1118 did to the ASan sibling and for the same reason: sccache hashes the preprocessed output, so a key without the stamp restores a pre-bump entry cleanly and misses on every TU that reaches a Vulkan header. Two consequences for this table. The rename starts a **fresh lineage**, so the first stamped entry holds one build's objects rather than an accumulation — expect it to measure *under* 2044 MiB at first, as `sccache-asan-windows-2025` did (1230 → 667 MiB), and do not read that drop as a saving. And the unstamped predecessor is collected by `cache-prune.yml`'s `-vk<version>` strip, which groups both spellings together, rather than being stranded for thirty days. |
| `sccache-asan-windows-2025-vk1.4.357.0` | **1492 MiB measured** 2026-09-13, against its 1500M cap — so this lineage has now also reached its cap, 825 MiB above the 667 MiB it held four days earlier. The 2026-09-09 reading was a *fresh* lineage's first banking, not its steady size, and reading a fresh lineage as a steady one is the mistake this row was written to prevent — it just made it in the other direction. **A lineage's size is only settled once it stops growing between measurements.** Previously **667 MiB measured** 2026-09-09 — down from **1230 MiB** on 2026-09-08, and that drop was the useful part. #1118 renamed the key to carry the SDK version, which started a **fresh lineage**: this entry holds one build's objects, where the old one had accumulated objects from many source generations in a single dir. **So an object set measured off a long-lived entry overstates what one build needs** — the `du -sm` 1241 MiB that rule 6's arithmetic used for this job was such a measurement. Its 1500M cap now has ~830 MiB spare. |
| **steady set** | **10376 MiB measured** 2026-09-13 by summing `size_in_bytes` over the API listing, **13 entries** — and the rows above reconcile: 3396 (Linux trio) + 3442 (subtotal) + 2046 + 1492 = **10376**. (`active_caches_size_in_bytes` reports 10 887 216 896 B = 10382 MiB; the floored per-row figures differ by rounding.) Against `cache-prune.yml`'s **20000 MiB** working ceiling that is **9624 MiB of headroom**. Two of the 13 entries — 1083 MiB — were stranded predecessors that the sweep could not group; the prefix-strip fix in this round collects them, taking the steady set to ~9293 MiB. **The set grew 7968 → 10376 MiB in four days** while nothing was deliberately added: two sccache lineages reached their caps (+825 and +2 MiB over their 09-09 readings) and #1205's re-key doubled vcpkg-Windows and stranded two predecessors. That is the shape to expect — a store grows by lineages filling and by key renames, not only by someone adding a cache. Previously **7968 MiB measured** 2026-09-09, 11 entries (the floored rows then summed to 7964). Against a ~9537 MiB wall and `cache-prune.yml`'s 8800 MiB working ceiling that is **832 MiB of headroom to the ceiling** — down from 1738 MiB on 2026-09-08, because #1118's 1300M cap cost the sanitizer trio ~1200 MiB and the Vulkan SDK bump stranded another 229. **Against the 25 GB cap set the same day it is ~17 GB** (see *The cap moved*): the measurement stands, the "nothing sizeable can be added" conclusion it carried does not. What binds is `cache-prune.yml`'s working ceiling, **20000 MiB**, leaving this set **12032 MiB** of room. That headroom, not any object set, is what a cap has to be sized against — see [sccache-cap-vs-object-set.md](sccache-cap-vs-object-set.md). |
