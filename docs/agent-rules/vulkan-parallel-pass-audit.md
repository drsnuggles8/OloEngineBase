...
| CSM, atlas and point-face shadows | Cascade or atlas-entry items; clear/transition shared atlas before fork | Atlas: implemented; atlas device checks, heavy multi-light scene. **CSM: recorded inline** since #1504 — forked cascades were followed by a device fault on a freed scene target ([vulkan-parallel-cascade-recording-fault.md](vulkan-parallel-cascade-recording-fault.md)); `OLO_VULKAN_PARALLEL_CSM=1` restores the fork for a driver re-test |
