...
| GPU fluid emit staging and body proxies | CPU per solver step, compute reads | Vulkan allocates one emit staging SSBO per pending batch and one body-proxy SSBO per nonempty step; earlier versions retire through deferred reclaim. The GL path retains its existing buffer. `EmitCount` comes from the Fluid UBO, so no CPU write to the GPU-produced counters is needed. Vulkan refuses `SeedParticles` after the first dispatch because a direct reset could race GPU output; recreate the solver. A delayed-frame fluid device test remains unrun. |
