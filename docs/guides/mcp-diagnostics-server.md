...
| `olo_renderer_support` | read-only: the active backend, render path and device capabilities, and the current static-mesh configuration and production presets evaluated through `RendererSupport`. The requested / capable / selected / produced / consumed stages are reported separately; selected, produced and consumed stay `null` until their own runtime oracles observe them, and presets for the inactive backend report unknown eligibility. No arguments |
