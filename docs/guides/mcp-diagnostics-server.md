...
| `olo_asset_open` | **(consented write)** open a file in the panel that edits it, through the same `EditorLayer::OpenAssetInEditor` dispatch as a Content Browser double-click: Sound Graph, Skill Tree, Visual Script (`.olovs`), Shader Graph, Dialogue, Cinematic, Shader, or a scene. `handle` or `path` (asset-relative). A type the Content Browser does not open (`.olomaterial`, `.olomat`) is refused with its type named. Unsaved changes in the target refuse unless `discardUnsaved:true` (the Content Browser asks with a native modal instead). `ok` is the panel's own readback: open AND reporting the requested file as loaded, after its deferred load had frames to run. See [Opening assets](#opening-assets-olo_asset_open) |
...
| `olo_gpu_resources` | **(issue #810)** the GPU objects that exist RIGHT NOW — textures, cubemaps, buffers, framebuffers, vertex arrays, shader programs — each carrying **both** identities: the RHI handle (index+generation, which survives a hot-reload and tells a recycled native name from a reused one) and the native id a RenderDoc / RGP capture shows. Textures report extent / mips / storage format, buffers their size. Works on **both backends**: under Vulkan the list is discovered from `RHI::ResourceRegistry` and enriched from the image / root-object registries, and `memoryHeaps` adds VMA's own per-heap usage-vs-budget — the number that answers "am I about to run out of VRAM?". Where `memoryHeaps` and `trackedBytes` disagree, believe the heaps: `trackedBytes` sums per-resource ESTIMATES that ignore tiling padding and suballocation slack. Sibling of `olo_memory_report`, which reports engine-tracked totals by category rather than the live object list. Debug-build instrument: a Release/Dist editor returns an empty list with a note |
...
| `olo_asset_import_settings` | **(consented write)** read (omit `settings`) or merge (provide `settings`) a **texture's** import settings in the `<image>.oloimport` sidecar the texture cook reads at pack build: `Format`, `ColorSpace`, `GenerateMips`, `AlphaMipChain` (spellings in the tool description, same as `TextureImportSettings.h`); a null value resets a field to Auto. A misspelt field or value, a sidecar the cook cannot parse, and a non-texture asset are all refused. `appliedByImporter` is true. Before #1453 this wrote JSON, which the cook rejected. Not undoable |
...
| `olo_camera_freeze_culling` | **(observer camera, issue #726)** freeze the camera used for **culling, LOD selection and Hi-Z occlusion** at its current pose while the viewport keeps rendering from the live camera. Fly away with `olo_camera_set_pose` / `olo_camera_orbit` and everything the frozen camera culled is *visibly missing* — the instrument for "is that object culled, or just not there?", and the only way to judge a cull from outside the camera that made it. The frozen frustum draws as a wireframe box, but **only** while shader debug draws are on, which is why the reply carries `shaderDebugDrawEnabled` (turn it on with `olo_shader_debug_draw {enabled:true}`). Freezing snapshots the pose **at the instant of the call**, so freeze first and pose afterwards. The reply's `cullFrustumCorners` are the 8 world-space corners, so an observer pose that actually contains the frozen volume can be computed rather than guessed. Call with no arguments to read state; **unfreeze when done** or every later screenshot keeps showing a stale cut. See [observer-camera.md](../agent-rules/observer-camera.md) |
...
| `olo_virtual_shadow_map_stats` | the VSM's cached previous-frame page-request/allocation/failure/draw/residency counters, including local-light subsets and cull overflow, plus physical-pool resolution/page count and owned VRAM bytes. The response distinguishes disabled/unavailable/no-data from a valid idle sample whose counters are zero |
| `olo_particle_stats` | **Did the particle renderer submit any draws this frame?** — the question a blank-particle bug asks first, and one nothing else could answer. `ParticleBatchRenderer::GetStats()` draw-call / instance counts alongside every emitter's live state (CPU `aliveCount`, on-device `gpuAliveCount`, the raw `gpuCounters` block, `gpuIndirectInstanceCount`, `deadGpuStages`, playing, render mode, distance vs `LODMaxDistance`), plus a `verdict` that names which side of the submit/rasterise line a blank frame failed on. **`olo_perf_snapshot` cannot substitute**: its `drawCalls`/`instancedDrawCalls`/`triangles` do not attribute particles at all — the same scene reads `0/14/256` on Vulkan and `24/6/127` on OpenGL while only OpenGL shows particles. Everything an investigation checks next (shaders, bindings, blend and depth state) is downstream of a draw that may never have happened, and a GPU-driven emitter's CPU counters are empty BY DESIGN, so the verdict says so rather than calling it a missing draw. Reads the counter and indirect SSBOs back, so it costs a frame flush. Issues #607, #1171 |
...
| `olo_benchmark_capture` | **(consented write)** run an issue-#974 benchmark capture manifest in the LIVE editor: open its scene, apply its renderer settings/exposure, pose each declared camera, warm the declared frames, capture every declared attachment (beauty + AOVs, PNG and Radiance `.hdr` at native resolution, including CPU-side `Derive:` lanes such as linear depth / roughness / metallic) and write the self-describing result directory (`manifest` path + optional `outDir`; default `assets/benchmark/captures/<Id>-editor-<backend>/`). This editor front door exists mainly to run manifests under `--rhi=vulkan`; the **deterministic** run-twice product is the test binary's `--olo-capture-manifest=` front door — this host pins the scene clock to the manifest's `Determinism` block like the test binary (issue #1470), but the editor owns its frame pacing and viewport-helper/tiering state per frame, and per-attachment dims in `result.json` record what was actually captured. Schema + products: [docs/guides/renderer-benchmarks.md](renderer-benchmarks.md). Gated behind **Agent writes** |
...
- **Not** writable, by design: per-tick runtime state (the `*StateComponent` family,
  ...
  member with no `OLO_PROPERTY` Get/Set pair. `TransformComponent.Rotation` is the
  exception: its annotated accessor pair exposes the stored Euler radians while
  keeping the authoritative quaternion synchronized. Also excluded is any
...
- **`space: "window"`** covers the whole editor dockspace: window-client pixels, which on
  ...
  framebufferScale}`, and an out-of-range point is refused with the same numbers. (Until #607
...
Since issue #821 these two are no longer special. Every lever is settable by name with
`olo_cvar_set { "name": "OLO_RG_POISON_TRANSIENTS", "value": "on" }`, from the editor's
**Window → Console** panel, or with `--set OLO_RG_POISON_TRANSIENTS=on` on the command line
of `OloEditor` / `OloRuntime` / `OloServer`. The transient-pool eviction that used to live
inside `olo_render_debug_set` is now a change callback on the aliasing lever, so every one
of those routes evicts correctly. `olo_render_debug_set` stays because it does two things a
generic setter cannot: it returns the `poisonColorMap` up front, and it settles two frames
for you. For anything else, reach for `olo_cvar_set`.
...
- **`renderpath`** — rendering path: `forward` | `forwardplus` | `deferred`.
  ...
  SSR / SSGI.
- **`msaa`** — deferred G-buffer sample count: `1` | `2` | `4` | `8` (clamped
  to the driver cap). Changing it rebuilds the render graph.
- **`persamplelighting`** — `off` | `on`; evaluates deferred lighting per MSAA
  sample. Changing it rebuilds the render graph.
...
- **`depthawareculling`** — `off` | `on`; switches clustered lighting between
  fixed-grid and 2.5D depth-aware culling without changing raster state.
- **`virtualshadowmaps`** — `off` | `on`; switches directional shadows between
  cascaded shadow maps and sparse Virtual Shadow Maps.
- **`vsmdebug`** — `off` | `cliplevel` | `pageaddress` | `residency` |
  `shadowtest` | `storeddepth` | `receiverdepth` | `shadowfactor`; selects the live VSM diagnostic view.
- **`ddgicascades`** — `off` | `on`; toggles camera-centred realtime-GI probe cascades.
- **`hzbocclusion`** — `off` | `on`; toggles GPU Hi-Z occlusion culling.
...
`olo_renderer_settings_set` owns discrete renderer choices and topology gates. Everything else in the
