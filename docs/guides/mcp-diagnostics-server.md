...
| `olo_rt_scene_stats` | The hardware ray-tracing scene (#978): whether ray query is usable and why not when it is not, resident BLAS population by geometry class, TLAS instances, acceleration-structure and scratch memory, compaction savings, and this frame's build/refit/compaction/retire counts. `unavailable` (no RT on this device) and `noData` (RT live, no TLAS built yet) are distinct |
