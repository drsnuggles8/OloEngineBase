...
|---|---|---|
| ~~`olo-gpu-amd`~~ | ~~`self-hosted,Linux,X64,gpu-amd`~~ | **RETIRED 2026-09-20.** Held no capability `olo-ci` lacks (same Unix user, same `video`/`render` groups, same `/dev/dri/renderD128`), while costing a third dispatch slot against an account slice sized for two heavy jobs. `gpu-conformance-amd.yml` requests `olo-ci`, and the registration is removed — see [§7 Retiring a runner](#7-retiring-a-runner) for the procedure. Everything below is kept for the provisioning detail, which still describes how the GPU is reached from this host; it is how a `olo-ci` job gets a hardware context today. |
