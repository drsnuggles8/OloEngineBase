...
4. External or cross-frame resources are imported; frame-local roots may still be declared as transients with explicit external backing when the graph owns their logical lifetime but not yet the physical surface creation.
...
- `RenderGraph::DeclareTransientFramebuffer(...)` now has a declaration-time external-backing overload for frame-local roots that should compile as transients but resolve to caller-owned backing. The transient planner reports those resources with `SkipReason = "external-backing"`, lifetime records keep them transient/non-imported, and pool materialization preserves their supplied backing instead of reallocating them.
- `RenderPipeline::PopulateBlackboard()` now uses that path for the deferred `GBufferResolved` / `GBufferMS` roots, so the graph compiler treats them as frame-local transients while `SceneRenderPass` continues to own the prepared runtime `GBuffer` helper that supplies the actual framebuffer objects.
- Dump/inspection visibility now matches the runtime semantics too: resource registry entries, unified lifetime records, `frameSummary`, and the JSON `resources` / `resourceLifetimes` arrays expose external-backing state explicitly instead of leaving it implicit in transient-plan skip reasons alone.
...
- The OIT depth attachment is now explicit in that contract too: `PopulateBlackboard()` publishes `OITDepthAttachment`, `OITPreparePass::Setup()` declares the `SceneDepthAttachment` -> `OITDepthAttachment` seed, and transparent OIT contributors declare `OITDepthAttachment` as a render-target read so fixed-function depth testing no longer hides behind execute-time framebuffer binds alone.
- `OITResolvePass` now mirrors that contract more strictly too: setup selects one canonical OIT framebuffer handle up front (the aliased `OITBuffer` published through `OITAccum` / `OITRevealage`), and execute no longer chooses between blackboard aliases on the fly.
- The old owner-backed OIT wrapper and transient-framebuffer override seam have been retired: `OITBuffer.{h,cpp}` is gone, `OverrideTransientFramebuffer(...)` has been removed from `RenderGraph`, and current frame-local roots now use an explicit declaration-time external-backing path instead of a post-declare override seam.
...
- `TAARenderPass` and `FogRenderPass` no longer keep pass-owned current-frame output fallbacks, and fog no longer keeps a pass-owned half-resolution scratch framebuffer. `TAAColor`, `FogColor`, and `FogHalfRes` are now required graph-owned execution surfaces, while the persistent temporal histories now live in renderer-owned external textures imported/extracted through explicit graph contracts instead of pass-owned framebuffer storage.
...
- Shadow resources are graph-owned or explicit external-backed transients consistently.
...
- `RenderPass` and `CommandBufferRenderPass` now inherit `RenderGraphNode` directly and surface declaration/work-type/async/side-effect metadata without an adapter layer. Production setup-callback plumbing has been removed from `RenderPass`; setup now lives in node-owned `Setup()` overrides or explicit test scaffolding. `PassGraphNode.h` has been deleted.
- The first production declaration-retirement sweep is in as well: `AOApply`, `Bloom`, `SSS`, `DOF`, `MotionBlur`, `TAA`, `Precipitation`, `Fog`, `ChromAb`, `ColorGrading`, `ToneMap`, `Vignette`, `FXAA`, `SelectionOutline`, `UIComposite`, and `Final` no longer seed legacy `DeclareRead()` / `DeclareWrite()` metadata in `Init()`. Their runtime resource contracts now come solely from `Setup()` blackboard declarations.
- That cleanup has now finished across the remaining runtime passes too (`Scene`, `Shadow`, deferred lighting/decal, AO, transparency, and stream-backed `SceneColor` modifiers): a grep over `OloEngine/src/OloEngine/Renderer/Passes/*.cpp` no longer finds production `DeclareRead()` / `DeclareWrite()` call sites at all. `RenderPass` itself no longer owns that legacy declaration API; the remaining static-declaration scaffolding now lives only in explicit node overrides and the test-only `OloEngine/tests/Rendering/LegacyDeclarativeRenderPass.h` shim.
...
- `EndScene()` now hands per-frame pass configuration, blackboard population, and GPU execution-state upload straight to `RenderPipeline` (`ConfigurePassesForFrame(...)`, `PopulateBlackboard(...)`, `UploadExecutionState(...)`), leaving `Renderer3DFrameExecution.cpp` focused on frame orchestration plus graph execution. The old post-build blackboard-handle refresh step is no longer part of current-frame execution.
...
- The render-stream bucket path has shed one more alias layer: `RenderPipeline` no longer stores a duplicate `StreamNodes` block alongside the actual pass owners. Stream routing, allocator/reset iteration, and parallel-submission lookup now resolve bucket-backed graph nodes directly from `FrameCorePasses.Scene` plus `RenderStreamPasses`, so there is no second ownership table to keep in sync.
...
- The remaining non-geometry stream-backed concrete passes now live under `RenderPipeline::RenderStreamPasses`, bundling `ForwardOverlay`, `Foliage`, `Water`, and `Decal` so render-stream routing resolves directly from the owning pass set instead of a second alias block.
- `RenderPipelineBuilderInternal::PrepareGraphNode(...)` now makes the production registration contract explicit too: the helper only borrows an existing graph node and synchronizes its graph-facing name before `AddNode(...)`; it no longer reads like an adapter or wrapper seam.
...
- `RenderPipelineInputs` no longer exposes one giant flat raw-pointer bag, and it no longer carries any runtime-settings pointer at all: the builder-facing surface is now just the owning `Passes` view plus the resolved `AOTechnique` enum needed to choose the SSAO-vs-GTAO registration path.
- The builder stage registrars no longer accept the whole `RenderPipelineInputs` bundle either: scene, transparency/AO, post, and render-stream registration now consume dedicated stage-input views, the transparency/AO stage sees only the resolved `AOTechnique` enum, and render-stream registration now routes directly from the owning pass set instead of a second `Nodes` alias block. Scene/post registration no longer pretends to depend on renderer, snow, fog, precipitation, or broader post-process runtime state it never reads.
...
- `SelectionOutlineRenderPass` now pushes one more piece of execute-time policy back into setup: the pass records both its late-post primary input handle and the secondary `SceneColor` entity-ID source during `Setup()`, so execution only resolves the already-chosen handles plus the graph-owned JFA scratch.
...
- Forward-path `Velocity` declaration now follows that same rule: `PopulateBlackboard()` sizes it from `SceneRenderPass::GetFramebufferSpecification()` and no longer gates the resource on `ScenePass::GetTarget()` being non-null before execution has even run.
...
- `TAA` and `Fog` no longer keep pass-owned persistent history framebuffers either: `RenderPipeline` now owns the external history textures, `PopulateBlackboard()` imports them directly when valid, registers graph-managed history sinks for both histories, and the passes declare next-frame history egress through explicit `ExtractHistoryTexture(...)` contracts instead of pass-local copy-back callbacks.
- `AOBuffer` is now graph-owned for both AO techniques, and the AO technique scratch is graph-visible too: `PopulateBlackboard()` declares `AOBuffer` as a graph-owned transient texture for GTAO (`R8`) and SSAO (`RG16F`), `GTAORenderPass` resolves graph-owned `GTAODenoisePing` / `GTAODenoisePong`, `GTAOEdge`, and `HZBDepth`, and `SSAORenderPass` resolves graph-owned `SSAORaw` and `SSAOBlur` instead of keeping current-frame blur storage privately owned.
- Blackboard declaration for the fullscreen/post chain is now keyed to real pass readiness, not settings alone: alongside the earlier `SSS` / `AOApply` / `Bloom` guards, `DOF`, `MotionBlur`, `TAA`, `Precipitation`, `Fog`, `ChromAb`, `ColorGrading`, `ToneMap`, `Vignette`, and `FXAA` now only materialize their graph-owned public outputs when their required shaders/UBOs are actually ready for execution.
- The scene-derived post inputs are no longer imported from scene/G-buffer attachments either: `SceneDepth`, `SceneNormals`, and `Velocity` are now graph-owned transient textures published by `SceneRenderPass` via `glCopyImageSubData()`. `SceneNormals` chooses `RG16F` in forward and `RGBA16F` in deferred to match the active source attachment, while the deferred per-sample path now forces a G-buffer resolve whenever AO, TAA, or motion blur needs current resolved scene-derived data.
- `SceneColor` is now graph-owned as the main scene MRT as well: `PopulateBlackboard()` declares the full scene framebuffer contract (`RGBA16F` HDR color + `R32Int` entity ID + `RG16F` normals placeholder + `RG16F` velocity + `Depth24Stencil8`), and `SceneRenderPass` resolves that blackboard framebuffer at execute time instead of importing `ScenePass->GetTarget()` as the canonical current-frame scene root.
- Deferred single-sample G-buffer exports now resolve through explicit framebuffer views over a graph-declared transient root with explicit frame-local backing instead of copy-out bridge textures or imported current-frame roots. `PopulateBlackboard()` declares `GBufferResolved`, publishes `SceneDepth`, `SceneNormals`, `GBufferAlbedo`, `GBufferNormal`, `GBufferEmissive`, and `Velocity` as attachment views, and lets `DeferredOpaqueDecalPass` remain the semantic producer for the post-decal attachment set without a duplicate export copy.
- Deferred multisample companions follow the same view model now too. `PopulateBlackboard()` declares `GBufferMS` with explicit backing, publishes `SceneDepthMS`, `VelocityMS`, and `GBuffer*MS` as explicit multisample attachment views, and `DeferredLightingPass::Setup()` now chooses one canonical MSAA-vs-resolved handle family up front so `Execute()` resolves that preselected set directly instead of re-deciding the path on the fly, while `DeferredOpaqueDecalPass` only performs real work when an export needs a copy rather than a same-storage view.
- The remaining imports in `PopulateBlackboard()` are now explicit long-lived/external resources (`Backbuffer`, IBL textures, persistent histories) rather than current-frame deferred/post/shadow roots.
- Generic texture/framebuffer extraction is now covered as a real build-time root too: `ComputeReachability()` seeds from extraction queues, `RGBuilder::ExtractTexture(...)` / `ExtractFramebuffer(...)` enqueue those roots during setup, and focused tests cover both direct pre-build extraction and builder-declared extraction preserving producer-only subgraphs.
- External sink modeling is now explicit as well: `RenderGraph::RegisterExternalTextureSink(...)` has typed texture/framebuffer overloads, `RGBuilder` can declare those sinks during setup, `ComputeReachability()` seeds from `m_ExternalTextureSinkContracts`, and focused tests/dumps cover `externalTextureSinkContractCount`, `externalTextureSinkContracts`, and `externalTextureSinks=` graph-digest metadata.
- Registration-order diagnostics are narrower now too: `RenderGraph::BuildFrameGraph()` compares simulated semantic ordering relations instead of raw differing direct-edge sets, so redundant transitive edges no longer count as `RegistrationOrderSensitivity` when explicit/setup-declared dependencies already fix the order.
- The production-shaped L5 hazard fixture now mirrors the real scene-chain `DependsOnPass(...)` contracts (`Shadow -> Scene`, `DeferredLighting -> ForwardOverlay`, `Scene/ForwardOverlay -> Foliage`, `Foliage -> Decal`, `Decal -> Water`) instead of preserving insertion-order assumptions in comments.
...
- The render graph now exposes framebuffer attachment views as first-class texture handles, and OIT uses them directly: `OITBuffer` remains the single MRT framebuffer contract while `OITAccum`, `OITRevealage`, and `OITDepthAttachment` are explicit attachment views instead of duplicate framebuffer aliases or implicit depth side effects.
- The same attachment-view machinery now covers framebuffer depth attachments too, so deferred `SceneDepth` / `SceneDepthMS` are modelled the same way as the color attachments they travel with instead of as separate export-only textures.
- `SceneColor` now publishes explicit live attachment views too (`SceneColorTexture`, `SceneEntityID`, `SceneViewNormals`, `SceneDepthAttachment`), so attachment consumers like water refraction/SSR setup, SSS bilateral filtering, and selection-outline JFA no longer index raw scene MRT attachments by hand at execute time.
- The graph-owned fullscreen/post outputs now publish explicit color attachment views as well (`SSSColorTexture`, `AOApplyColorTexture`, `BloomColorTexture`, `DOFColorTexture`, `MotionBlurColorTexture`, `TAAColorTexture`, `PrecipitationColorTexture`, `FogColorTexture`, `ChromAbColorTexture`, `ColorGradingColorTexture`, `ToneMapColorTexture`, `VignetteColorTexture`, `FXAAColorTexture`, `SelectionOutlineColorTexture`, `UICompositeTexture`, plus the alias `PostProcessColorTexture`). `RenderPipelineBuilderInternal` now stores paired framebuffer+texture input handles for fullscreen/post passes, setup declares direct reads of the chosen texture views, and execute-time sampling across `AOApply`, `Bloom`, `DOF`, `MotionBlur`, `TAA`, `Precipitation`, `Fog`, the late post chain, `SelectionOutline`, `UIComposite`, and `Final` resolves those explicit texture views instead of manually pulling color attachment 0 from the chosen input framebuffer.
- `RenderGraph::BuildFrameGraph()` now expands parent-framebuffer access and feedback declarations onto matching attachment-view resources during compilation, which keeps parent-writer -> attachment-view-reader flows visible to dependency derivation, reachability, barrier planning, transitions, lifetimes, and hazard validation without preserving the old paired parent-framebuffer bridge read in fullscreen/post setup.
- `RenderGraph` now exposes single-mip, array-layer, cube-face, and multisample-resolve texture views via `CreateTextureMipView(...)`, `CreateTextureArrayLayerView(...)`, `CreateTextureCubeFaceView(...)`, and `CreateTextureMultisampleResolveView(...)`. `RenderPipeline::PopulateBlackboard()` publishes `HZBDepthMipN` handles for the graph-owned HZB pyramid, explicit shadow-map layer/face handles over graph-declared shadow roots with explicit frame-local backing, and deferred canonical single-sample `SceneDepth` / `SceneNormals` / `GBuffer*` / `Velocity` handles as resolve views over their `*MS` sources when MSAA is active. The remaining view-related work is now broader production adoption of versioned-write semantics rather than another missing view family.
- `RGBuilder` now supports opt-in explicit versioned writes for textures, framebuffers, and buffers via `WriteNewVersion(...)`. Those calls clone the source descriptor into graph-owned pass-derived handles and let `BuildFrameGraph()` treat sequential rewrite chains as explicit producer-owned resources instead of leaning only on mutable canonical names. Production now uses that path in fifteen safe seams: `SelectionOutlineRenderPass::Setup()` publishes a producer-owned version of `SelectionOutlineColor` for `UICompositeRenderPass`, `SSSRenderPass::Setup()` publishes a producer-owned version of `SSSColor` that `AOApplyRenderPass` and `BloomRenderPass` now prefer when later seams are absent, `AOApplyRenderPass::Setup()` publishes a producer-owned version of `AOApplyColor` that `BloomRenderPass` now prefers, `BloomRenderPass::Setup()` publishes a producer-owned version of `BloomColor` that `DOFRenderPass`, `MotionBlurRenderPass`, `TAARenderPass`, `PrecipitationRenderPass`, `FogRenderPass`, `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `DOFRenderPass::Setup()` publishes a producer-owned version of `DOFColor` that `MotionBlurRenderPass`, `TAARenderPass`, `PrecipitationRenderPass`, `FogRenderPass`, `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `MotionBlurRenderPass::Setup()` publishes a producer-owned version of `MotionBlurColor` that `TAARenderPass`, `PrecipitationRenderPass`, `FogRenderPass`, `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `TAARenderPass::Setup()` publishes a producer-owned version of `TAAColor` that `PrecipitationRenderPass`, `FogRenderPass`, `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `PrecipitationRenderPass::Setup()` publishes a producer-owned version of `PrecipitationColor` that `FogRenderPass`, `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `FogRenderPass::Setup()` publishes a producer-owned version of `FogColor` that `ChromaticAberrationRenderPass`, `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `ChromaticAberrationRenderPass::Setup()` publishes a producer-owned version of `ChromAbColor` that `ColorGradingRenderPass`, `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `ColorGradingRenderPass::Setup()` publishes a producer-owned version of `ColorGradingColor` that `ToneMapRenderPass`, `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `ToneMapRenderPass::Setup()` publishes a producer-owned version of `ToneMapColor` that `VignetteRenderPass`, `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `VignetteRenderPass::Setup()` publishes a producer-owned version of `VignetteColor` that `FXAARenderPass`, `SelectionOutlineRenderPass`, and `UICompositeRenderPass` now prefer when later seams are absent, `FXAARenderPass::Setup()` publishes a producer-owned version of `FXAAColor` that `SelectionOutlineRenderPass` and `UICompositeRenderPass` now prefer when available, and `UICompositeRenderPass::Setup()` publishes a producer-owned version of `UIComposite` for `FinalRenderPass`. Canonical base-name `Get*Handle(...)` lookup now follows the latest explicit version too, so debug/frame-capture/external name-based resolve stays aligned with those producer-owned outputs.
- `RenderGraphDebugger` no longer assumes every visible graph entry is a `RenderPass`. The debugger and DOT export now walk `RenderGraphNode` entries directly, keep non-pass graph nodes visible in the UI, and only downcast when optional framebuffer details are available from a render-pass-derived node.
- `RGBuilder` now supports explicit same-pass feedback declarations via `AllowFeedback(...)`, and the real scratch/accumulation passes that intentionally read and rewrite the same resource inside one node now annotate that contract directly in setup (`BloomMips`, `FogHalfRes`, `SSAORaw`, `WaterRefraction`, `JFAPing/JFAPong`, `HZBDepth`, `GTAOEdge`, `GTAODenoisePing`, `GTAODenoisePong`, and weighted-blended OIT accum/revealage contributors). The `SceneColor` read-modify-write nodes (`WaterRenderPass` and `OITResolveRenderPass`) now declare that feedback explicitly too, because parent-framebuffer declarations compile through to the attachment views they sample. The full `SceneColor` modifier chain (`DeferredLighting`/`Scene` -> `ForwardOverlay` -> `Foliage` -> `Decal` -> `Water` -> `Particle` -> `OITResolve`) now also pins consecutive modifier ordering with explicit `DependsOnPass(previous_writer)` edges via the new `SetPreviousSceneColorWriter(...)` setter wired in `RenderPipelineBuilderScene::RegisterRenderStreamNodes`. The OIT accumulation chain (`OITPrepare` clear -> `Decal` OIT path -> `Particle` OIT path) is similarly pinned: both contributors emit `DependsOnPass("OITPreparePass")` in their OIT branches, and `Particle` carries an additional `SetPreviousOITContributor(...)` setter wired by `RenderPipelineBuilderTransparency::RegisterTransparencyAndAONodes`. Together these mean registration order in the scene and transparency builders is no longer load-bearing for either canonical-name RMW chain (`SceneColor`, `OITAccum`/`OITRevealage`).
- The late scene-modifier chain is a little less execute-driven now too: `DecalRenderPass::Setup()` declares the `SceneDepth` shader-sample contract needed for decal projection, and the focused hazard suite now asserts that compiled setup declarations derive the `SceneDepthProducer -> DecalPass` edge rather than relying on execute-time depth resolution alone.
- The OIT depth-seed path is setup-visible now too: `OITPrepareRenderPass::Setup()` declares the `SceneDepthAttachment` transfer-source read plus the `OITDepthAttachment` transfer-destination write, `ParticleRenderPass` / `DecalRenderPass` declare the fixed-function `OITDepthAttachment` read they depth-test against, and the focused hazard suite now asserts the derived `ScenePass -> OITPreparePass -> ParticlePass` chain.
- The live `SceneColor` modifier chain is now more graph-native too: `ForwardOverlayRenderPass`, `FoliageRenderPass`, the non-OIT `DecalRenderPass` path, and the classic `ParticleRenderPass` path all declare explicit render-target read/modify/write feedback on `SceneColor` in `Setup()`, while `WaterRenderPass` now relies on its `SceneColorTexture` / scene-attachment reads plus compiled parent->view expansion instead of a per-frame upstream-order side channel. The old `SetHasForwardOverlayCommands(...)` / `SetUpstreamCommandAvailability(...)` plumbing and its `RenderPipeline::ConfigurePassesForFrame()` wiring have been removed.
- Those simple scene modifiers now carry their scene framebuffer choice through setup as well: `ForwardOverlayRenderPass`, `FoliageRenderPass`, `WaterRenderPass`, `ParticleRenderPass`, and `DecalRenderPass` store the canonical `SceneColor` framebuffer handle during `Setup()` and resolve that selected handle in `Execute()` instead of replaying the same blackboard lookup there.
- The same execute-time cleanup now covers the OIT target side too: `OITPrepareRenderPass`, `ParticleRenderPass`, and `DecalRenderPass` store the chosen `OITBuffer` framebuffer handle during `Setup()` and resolve that stored target in `Execute()` instead of re-querying the blackboard for the same OIT framebuffer each frame.
- The remaining stable scene-target resolution is now setup-owned too: `SceneRenderPass` carries the canonical `SceneColor` framebuffer handle selected during `Setup()` into `Execute()`, and `DeferredLightingPass` now resolves its scene target from the same setup-selected handle state rather than re-reading the blackboard there.
- `RenderPass` now carries a setup-owned primary output framebuffer handle too, with a matching graph test that resets it between frames; the single-output fullscreen/post family (`AOApply`, `SSS`, `DOF`, `MotionBlur`, `TAA`, `Precipitation`, `ToneMap`, `FXAA`, `ColorGrading`, `ChromaticAberration`, `Vignette`, `UIComposite`) now stores its canonical output target during `Setup()` and resolves that stored handle in `Execute()` instead of re-querying the blackboard for the same destination every frame.
- That same output-handle contract now covers the remaining scratch framebuffer families too: `SSAORenderPass` stores the graph-owned `SSAORaw` scratch framebuffer in setup-owned state, `BloomRenderPass` stores its bloom-mip framebuffer chain during `Setup()`, and `FogRenderPass` / `SelectionOutlineRenderPass` now store their half-res and JFA ping-pong scratch framebuffers during `Setup()` as well. There are now no direct `ResolveFramebuffer(board->...)` lookups left under `OloEngine/src/OloEngine/Renderer/Passes`.
- The same cleanup now covers canonical secondary textures and export surfaces too: `AOApply`, `DOF`, `MotionBlur`, `TAA`, `SSS`, `SSAO`, `Fog`, `SelectionOutline`, `Water`, `Decal`, `OITResolve`, `GTAO`, `SceneRenderPass`, and `DeferredOpaqueDecalPass` all store their selected scene/OIT/history/AO/entity/refraction/shadow/HZB/export handles during `Setup()` and resolve those stored handles in `Execute()`. There are now no direct `ResolveTexture(board->...)` lookups and no execute-time `GetBlackboard()` call sites left under `OloEngine/src/OloEngine/Renderer/Passes`, so the remaining modernization work in this area is no longer pass-local blackboard resource re-selection.
