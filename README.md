...
- **Steamworks SDK** — *optional*, and **you must download it yourself**: its licence forbids redistribution, so it is neither in this repo nor available through vcpkg. Needed only to work on Steam features (achievements, Steam Cloud, rich presence/overlay). Steam support is **auto-detected**: `OLO_WITH_STEAM` turns on when `STEAMWORKS_SDK_ROOT` points at a real SDK (a `public/steam` directory) and stays off otherwise — so without the SDK everything builds, runs and tests exactly as before, and with it installed you get the Steam path without passing a flag. The SDK itself is free — an ordinary Steam account and the Steamworks SDK Access Agreement, not the $100 Steam Direct publishing fee. Setup: [docs/ops/build.md](docs/ops/build.md#steamworks-sdk-optional--you-must-obtain-it-yourself).
