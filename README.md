...
- **Steamworks SDK** — *optional*, and **you must download it yourself**: its licence forbids redistribution, so it is neither in this repo nor available through vcpkg. Needed only to work on Steam features (achievements, Steam Cloud, rich presence/overlay); Steam support is OFF by default and everything builds, runs and tests without it. The SDK itself is free — an ordinary Steam account and the Steamworks SDK Access Agreement, not the $100 Steam Direct publishing fee. Setup: [docs/ops/build.md](docs/ops/build.md#steamworks-sdk-optional--you-must-obtain-it-yourself).
