...
Deterministic benchmark scenes with checked-in capture manifests
(`OloEditor/assets/benchmark/manifests/`). Captured through
`OloEngine-Tests.exe --olo-capture-manifest=<manifest>`; contract and schema in
[docs/guides/renderer-benchmarks.md](../../../../docs/guides/renderer-benchmarks.md).
Unlike everything above, these scenes exist to answer "is the frame *good* and
which term is responsible?", not "did it change?".

### [Benchmark/MaterialLab.olo](Benchmark/MaterialLab.olo)

**Purpose**: material-response laboratory — numerical ground truth for the PBR
pipeline. GENERATED by `OloEngine/tests/scripts/generate_material_lab_scene.py`
(edit the generator, not the file).
**Contents**: 10×10 roughness×metallic sphere sweep (ClosureV2), an 18% grey
card + linear colour-checker strip, an emissive row, a normal-map row
(rusted-iron/wall/gold), white sun + Newport Loft IBL, tonemap None.
**Pass**: the reference patches read their authored values —
`MaterialLabSceneEvidenceTest` asserts neutrality of the grey card, the
achromatic ladder, channel dominance, mirror-vs-rough contrast and emissive
contrast on the rendered frame.
**Fail**: a tinted grey card (a coloured light leaked in), a flat metal row
(roughness not reaching the shader), emissive spheres matching their control.

### [Benchmark/MaterialLabFurnace.olo](Benchmark/MaterialLabFurnace.olo)

**Purpose**: white-furnace variant — energy-conservation reading. Same sweep
with albedo 1.0 under a uniform-white 1×1 HDRI
(`assets/textures/hdr/furnace_white_1x1.hdr`), no analytic light.
**Pass**: an energy-conserving material is (near-)indistinguishable from the
background; any sphere reading darker than the furnace is losing energy, any
brighter is gaining it.

### [Benchmark/Courtyard.olo](Benchmark/Courtyard.olo)

**Purpose**: dense architectural / foliage feature-stress scene — the
everything-at-once frame. GENERATED by
`OloEngine/tests/scripts/generate_courtyard_scene.py`.
**Contents**: Sponza + 142 point lights + 12 spots (10 shadow-casting locals),
realtime-DDGI probe volume, SSGI + SSR + volumetric fog, four decals (one per
mode), terrain patch with wind-blown masked foliage + impostors, a 48-tower
instanced ring as distant geometry. Deferred path (declared in its manifests).
**Pass**: the golden/diagnostic captures show lit vaults (DDGI), visible fog
shafts, all four decal modes projected, foliage swaying, towers fading into fog.
**Fail**: a black atrium (DDGI off — the renderer flag comes from the MANIFEST,
not the scene), missing decals, dead foliage.

### [Benchmark/OceanCoast.olo](Benchmark/OceanCoast.olo)

**Purpose**: ocean / coast hero scene — the publication-quality water frame.
**Contents**: FFT ocean (JONSWAP, seed 974, 3 cascades), a procedural island
(Drift Ridgeback's shaping recipe, `ShapingIslandFalloff` on), palm foliage, the
Kenney ship (CC0) anchored offshore, paused golden-hour time-of-day, light warm
haze. Forward path.
**Pass**: sun glint runs toward the hero camera with the ship silhouetted on it;
the shoreline shelves (no vertical wall — the Drift #880 contract); the
diagnostic manifest's waterline/submerged/overhead cuts all read correctly.
**Fail**: water reading as a dark void (nothing to reflect), a terrain cut at
the waterline, foam/aliasing artifacts on the glint.

---

## Reference fixtures (issue #1239)

The character-and-flora half of the benchmark set: a reference head, a short- and a
long-coated animal, a meadow and a woodland. GENERATED by
`OloEngine/tests/scripts/generate_reference_fixture_scenes.py` (edit the generator, not
the files). Manifests are `OloEditor/assets/benchmark/manifests/{reference-head,
animal-short-coat, animal-long-coat, meadow, woodland}.diagnostic.yaml`, each with four
pinned sequences — **frontal, grazing, backlit and moving**.

These fixtures deliberately **show current limitations**: no subsurface scattering on the
head, a solid shell instead of a groom on the long coat, alpha-masked cards instead of
geometry in the meadow. That is the baseline epic #1225's feature work is measured
against, so "it looks unfinished" is the expected reading, not a bug to fix here.
Contract, provenance schema and the measured budgets:
[docs/guides/benchmark-reference-fixtures.md](../../../../docs/guides/benchmark-reference-fixtures.md).

### [Benchmark/ReferenceHead.olo](Benchmark/ReferenceHead.olo)

**Purpose**: head-scale shading reference under a pinned studio rig.
**Contents**: Suzanne (CC0, organic head geometry) and DamagedHelmet (CC-BY 4.0, 2K PBR
textures) on matte plinths; one shadow-casting key light, a cool rim behind, a warm fill.
Deferred path, SSR on, no fog.
**Pass**: contact shadows sit under the plinths; the grazing camera shows a clean
specular falloff across Suzanne's cheek; the backlit camera gives a rim without blowing
out.
**Fail**: shadow detached from its caster (a 10x `ShadowNormalBias`, #1119); the helmet's
normal map flat; either subject blown white at the fixed exposure.

### [Benchmark/AnimalShortCoat.olo](Benchmark/AnimalShortCoat.olo)

**Purpose**: short-coated animated character reference.
**Contents**: Khronos Fox, Walk clip playing, scaled 0.01 (the asset is authored in
centimetres) and rotated to face +X. Low sun from behind-left.
**Pass**: the skeleton animates in edit mode; the velocity AOV on the moving camera
carries both camera parallax and skinned deformation.
**Fail**: a static bind pose (animation not previewing); a 100x mis-scaled subject.

### [Benchmark/AnimalLongCoat.olo](Benchmark/AnimalLongCoat.olo)

**Purpose**: the no-groom baseline. Same ground, sky, rig and camera poses as
AnimalShortCoat, so the two captures isolate the coat.
**Contents**: procedural quadruped whose long coat is a solid offset shell
(`Models/Reference/generate_reference_animals.py`).
**Pass**: the coat reads as closed opaque geometry with a hard silhouette — that IS the
measurement.
**Fail**: anything that makes the coat look good. Improving it here destroys the baseline
#1232/#1241 are measured against.

### [Benchmark/Meadow.olo](Benchmark/Meadow.olo)

**Purpose**: open ground-cover vegetation reference.
**Contents**: 192 m procedural terrain with three wind-driven alpha-masked billboard
layers (grass, tall grass, flowers). Deferred, volumetric fog, wind on.
**Pass**: the field is dense and the wind bend is identical run to run; the grazing camera
shows the cards turning edge-on.
**Fail**: a camera below the terrain; a static field (wind not reaching the mock clock).
