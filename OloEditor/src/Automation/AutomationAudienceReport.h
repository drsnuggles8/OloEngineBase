...
// Pure, engine-light rendering of a structured command payload into the
// HUMAN-facing content block of a dual-audience result (issue #673;
// MCP spec 2025-06-18 `Annotations.audience` on a ContentBlock).
//
// Lives in the automation layer, not under MCP/, because it renders the RESULT
// PAYLOAD and knows nothing about a transport (issue #1123). MCP/McpAudienceReport.h
// is a forwarding header for the call sites that still spell the old path.
//
...
// pretty-printed block of AutomationResult::Structured() did.
//
...
namespace OloEngine::Automation::AudienceReport
{
