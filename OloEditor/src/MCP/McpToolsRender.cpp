...
namespace OloEngine::MCP
{
    namespace
    {
        ...
        ToolResult Handle_RenderFrameBreakdown(IAutomationHost& host, const Json& args)
        {
            ...
            // GENERATION, not the retained count (#1510): the manager keeps at most
            // GetMaxCapturedFrames() frames, so once it is full a new capture evicts
            // the oldest, the count stays put, and every call after the 60th timed
            // out against a live viewport. Same wait as Handle_PerfCaptureFrame.
