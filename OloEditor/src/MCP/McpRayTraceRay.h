...
namespace OloEngine::MCP::RayTraceRay
{
    ...
    {
        if (!args.contains("rays") || args["rays"].is_null())
            return "Missing required argument 'rays': an array of {origin, direction} world-space rays.";
        const Json& rays = args["rays"];
    ...
    [[nodiscard]] inline Json InputSchema()
    {
        return Schema::Object()
            .Prop("rays", Schema::Array(Schema::Object()
                              ...
                              .Desc("The rays to trace, 1 to 64 per call."))
    ...
    [[nodiscard]] inline Json OutputSchema()
    {
        return Schema::Object()
            ...
            .Required({ "status", "batchId", "rayCount", "flags" });
    }
} // namespace OloEngine::MCP::RayTraceRay
