...
namespace OloEngine
{
    ...
    void SceneHierarchyPanel::DrawComponents(Entity entity)
    {
        ...
        DrawComponent<LODGroupComponent>("LOD Group", entity, [entity](auto& component)
            ...
            ImGui::Separator();
            // m_AutoGenerated records PROVENANCE, so only the two actions that change
            // where a level's geometry came from touch it: Generate LODs sets it, and
            // re-pointing a level at a real asset clears it. Distance drags, adds and
            ...
            {
                ImGui::TextDisabled("Selected by camera distance (no generated error data).");
            }
            ...
            for (sizet i = 0; i < component.m_LODGroup.Levels.size(); ++i)
            {
                    ...
                    if (ImGuiPayload const* const payload = ImGui::AcceptDragDropPayload("CONTENT_BROWSER_MODEL"))
                    {
                        ...
                        if (assetManager)
                        {
                            ...
                            if (handle != 0)
                            {
                                ...
                                // This level now names a real, file-backed asset, so
                                // the chain is no longer purely generated. Once every
                                // level is like this the serializer can persist them
                                // verbatim; until then the remaining memory-only
                                // handles still force the derived path.
                                component.m_AutoGenerated = false;
                            }
                        }
                    }
                    ImGui::EndDragDropTarget();
                ...
                if (!Math::BitwiseEqual(level.MaxDistance, prevDistance))
                {
                    needsResort = true;
                }

                ImGui::Text("Triangles: %u", level.TriangleCount);
                // Generated error is read-only: it is a measurement of the
                // simplification that produced this level, not a tuning knob.
                ImGui::Text("Error: %.4f", static_cast<double>(level.Error));
            ...
            if (entity.HasComponent<MeshComponent>())
            {
                ...
                if (meshComp.m_MeshSource && meshComp.m_MeshSource->GetSubmeshes().Num() == 1)
                {
                    ...
                    if (ImGui::Button("Generate LODs"))
                    {
                        ...
                        // distances. Same generator the import path runs.
                        //
                        // Bias is a user setting, not part of the generated chain, so
                        // it is carried across the wholesale group replacement below —
                        // otherwise pressing this button silently resets it to 1.0.
                        f32 const previousBias = component.m_LODGroup.Bias;
                        component.m_LODGroup = MeshOptimization::GenerateAutoLODGroup(
                            *meshComp.m_MeshSource, baseHandle);
                        component.m_LODGroup.Bias = previousBias;
                        // Derived data: the levels name memory-only assets, so the
                        // scene serializer regenerates rather than persists them.
