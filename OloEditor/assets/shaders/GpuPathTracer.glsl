...
// ---------------------------------------------------------------------------
// What the scene looks like along a ray — include/RayTracedSurfaceHit.glsl.
//
// The material-texture table and its heap sampler, the glTF MASK alpha test,
// the candidate-solid rule, the ray flags, the analytic tangent frame, the
// triangle fetch, the closest-hit resolve and the segment occlusion query all
// moved there for issue #1169, together with the include ORDER for
// LightSampling.glsl and RayTracingAlphaTest.glsl, which is load-bearing and
// not obvious.
//
// THE TIER THAT IS VALIDATED AGAINST THIS ORACLE MUST NOT CARRY ITS OWN COPY OF
// WHAT THE ORACLE SEES. #1140 established that for the light densities; a ray
// hit is the same thing one level up. If ReSTIR GI's bounce interpolated its
// normals, snapped them to the geometric side or handled an unshadeable slot
// even slightly differently, the two would shade the same vertex differently
// and the convergence test would be measuring THAT rather than the estimator.
//
// The Pt* names below are ALIASES, not wrappers: the integrator after this
// point is unchanged, and the compiled SPIR-V is byte-identical across the move
// — checked with glslc before and after, the way #1140 proved the oracle
// unmoved when the light densities were extracted.
// ---------------------------------------------------------------------------
#include "include/DescriptorHeapTextures.glsl"

#define OLO_RT_HIT_TLAS_ADDRESS u_TlasAddress.xy
#define OLO_RT_HIT_INSTANCE_MASK u_TlasAddress.z
...
#define OLO_RT_HIT_TEXTURES_REQUESTED ((u_EmissiveTable.w & OLO_PT_FLAG_TEXTURES) != 0u)
#include "include/RayTracedSurfaceHit.glsl"

#define OloPtMaterialTextures OloRtMaterialTextures
#define PtTexturesEnabled OloRtTexturesEnabled
#define PtMaterialTextures OloRtMaterialMaps
#define PtSampleMaterialTexture OloRtSampleMaterialTexture
#define PtSampleAlpha OloRtSampleAlpha
#define PtCandidateIsSolid OloRtCandidateIsSolid
#define PtRayFlags OloRtRayFlags
#define PtApplyNormalMap OloRtApplyNormalMap
#define PtHit OloRtHit
#define UnshadeableHit OloRtUnshadeableHit
#define FetchTriangle OloRtFetchTriangle
#define TraceClosestGeometry OloRtTraceClosestGeometry
#define TraceClosest OloRtTraceClosest
#define IsOccluded OloRtIsOccluded
