// =============================================================================
// Water_OIT.glsl — Weighted-blended OIT variant of Water.glsl.
//
// Vertex / tessellation stages are byte-for-byte identical to Water.glsl.
// Fragment stage emits accum + revealage into the OITBuffer attachments so
// overlapping semi-transparent water surfaces composite without requiring
// back-to-front sorting. Transparency is taken from `u_WaterColor.a`
// (forward Water.glsl hardcodes 1.0 because its path is effectively opaque
// via refraction compositing — the OIT variant honours the user-set alpha).
//
// Selected by WaterRenderPass when
// `RendererSettings::DeferredSettings::OITEnabled` is true.
// =============================================================================
...
// funcs configured by WaterRenderPass in OIT mode.
...
void main()
{
    ...
    // --- Final alpha ---
    // OIT variant honours the user-set water alpha (u_WaterColor.a). For
    // shoreline / shallow regions the depth fade further attenuates the
    // alpha so the underlying seabed / refraction can show through cleanly.
    float transparency = clamp(u_WaterColor.a, 0.0, 1.0);
    transparency *= clamp(0.2 + 0.8 * depthFade, 0.0, 1.0);

    if (transparency < 0.001)
        discard;

    // View-space depth used by the WB-OIT weight. Reconstruct from the
    // fragment's clip-space w (equivalent to linear view z for a
    // perspective projection).
    vec4 clipPos = u_Projection * u_View * vec4(v_WorldPos, 1.0);
    float viewZ = max(clipPos.w, 1e-3);
    float weight = ComputeOITWeight(transparency, viewZ);
    OITPack(finalColor, transparency, weight, o_Accum, o_Revealage);
}
