...
// ever enters the accumulator. This pass resolves it into SSRResolved, which
// the graph copies into the SSRHistory sink and draw C composites.
//
// WHY THE SPLIT EXISTS AT ALL: SSR used to composite straight into the scene
...
// this resolve, never before it. It is also what pays for the raised
// SSRMaxRoughness: one VNDF sample per pixel can carry a wide lobe once
// something is averaging it.
//
...
// The SAME std140 block PostProcess_SSR.glsl declares — one upload feeds all
// three draws of the pass. Mirrored on the CPU by SSRUBOData.
...
const float SSR_DEPTH_TOLERANCE = 0.05;
...
void main()
{
    ...
    float confidence = OloTemporalDepthConfidence(current.a, history.a, SSR_DEPTH_TOLERANCE);
    ...
    // NOT clamped to non-negative: the SSR signal is a signed DELTA — a
    // reflection darker than the surface it replaces is a legitimate negative,
    // and clamping it would leave a mirror unable to darken anything. Alpha
    // carries THIS frame's depth forward so next frame's disocclusion test has
