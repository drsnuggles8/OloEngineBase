...
vec3 OloReSTIROctDecode(vec2 e)
{
...
vec3 OloReSTIRViewPosFromDepth(vec2 uv, float depth)
{
...
// and it is why RayOriginNormalBias exists as a tunable.
OloReSTIRSurface OloReSTIRLoadSurface(vec2 uv, float depth, vec4 packedNormal, vec4 packedAlbedo, int pbrModel)
{
    OloReSTIRSurface s;
    s.Valid = true;
    vec3 viewPos = OloReSTIRViewPosFromDepth(uv, depth);
    s.Position = (u_InvView * vec4(viewPos, 1.0)).xyz;
    s.ViewDepth = -viewPos.z;
    s.ShadingNormal = OloReSTIROctDecode(packedNormal.xy);
...
OloReSTIRSurface OloReSTIRInvalidSurface()
{
    OloReSTIRSurface s;
    s.Valid = false;
    s.Position = vec3(0.0);
