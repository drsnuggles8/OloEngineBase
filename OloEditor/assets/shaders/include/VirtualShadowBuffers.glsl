...
layout(std430, binding = 55) coherent buffer VSMMetaTable { uint b_MetaTable[]; };

// Hierarchical Page Buffer: VSM_HPB_MIP_COUNT mips of the DIRTY flag per clip
// level, packed by vsmHPBIndex().
layout(std430, binding = 56) coherent buffer VSMHierarchicalPageBuffer { uint b_HPB[]; };
...
layout(std430, binding = 58) coherent buffer VSMRequests
{
    uint b_RequestCount;
...
// real pressure.
layout(std430, binding = 59) coherent buffer VSMFreePages
{
    uint b_FreeCount;
...
layout(std430, binding = 60) readonly buffer VSMInvalidations
{
...
layout(std430, binding = 61) readonly buffer VSMCullInstances { VSMCullInstance b_CullInstances[]; };
...
layout(std430, binding = 64) coherent buffer VSMDrawCommands { VSMDrawCommand b_DrawCommands[]; };
...
layout(std430, binding = 65) coherent buffer VSMStatistics
{
