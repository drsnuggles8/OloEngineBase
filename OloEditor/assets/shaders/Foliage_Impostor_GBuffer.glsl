...
//
// Identical vertex stage and identical atlas sampling (3-frame octahedral
// blend + single-step depth parallax, issue #433). The only difference is the
// fragment's output: instead of relighting the card itself from one directional
// light and compositing into SceneColor, it writes the G-Buffer MRT and lets
// DeferredLightingPass shade it like any other opaque surface.
//
// Why this exists (#1225): the forward impostor card is drawn AFTER
// DeferredLightingPass, so it never appears in GBufferAlbedo / GBufferNormal.
// Everything derived from the G-Buffer therefore treats the canopy as empty
// sky — SSAO, SSGI and SSR get no canopy occluder, and the forest floor is lit
// like open ground. It also meant the canopy saw exactly ONE light (u_Light0)
// plus a flat 0.3 ambient, while every other surface in the scene got the full
// deferred light list, shadows and IBL.
//
// This is safe to do because the impostor path was never alpha-BLENDED: the
// card is opaque alpha-TESTED (`coverage < cutoff -> discard`) with
// depth-write on and blending explicitly disabled by its render state, which
// is exactly the contract a G-Buffer MRT write requires. The distance fade,
// which the forward path folded into the output alpha, becomes a hard discard
// here — the same collapse Foliage_Instance_GBuffer already makes.
//
// Keep this file's vertex stage and atlas-sampling body in step with
// Foliage_Impostor.glsl; they are the same card and must agree on where it is
// and what it samples, or the two paths disagree at the Forward/Deferred seam.
// =============================================================================
...
// Unlike the forward variant, the fragment stage here DOES read
// v_InstanceIndex (for u_EntityID), so the varying is forwarded.
...
void main()
{
    ...
    // Distance fade. The forward card folds this into the output alpha; the
    // G-Buffer has no alpha blending, so it collapses to a hard discard — the
    // same threshold Foliage_Instance_GBuffer uses.
    float distFade = 1.0 - smoothstep(u_FadeStart, u_ViewDistance, dist);
    if (coverage * distFade < 0.3)
        discard;

    // The baked object-space normal, rotated into world space by the instance
    // rotation. This is the normal the forward card relit itself with; here it
    // becomes the G-Buffer normal and DeferredLightingPass does the shading —
    // so the canopy now receives every light, shadows and IBL rather than the
    // single directional light the forward path hardcoded.
