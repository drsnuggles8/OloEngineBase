...
using OloHeaderTool::ComponentSerInfo;
using OloHeaderTool::PropType;
using OloHeaderTool::SerField;
using OloHeaderTool::Detail::PeelSerializeMarker;
using OloHeaderTool::Detail::ReplaceAll;
using OloHeaderTool::Detail::StripPrefix;
using OloHeaderTool::Detail::Trim;
...
static void EmitComponentTypeVisitor(std::ostream& out, const std::set<std::string>& componentNames)
{
    out << "// Auto-generated by OloHeaderTool — DO NOT EDIT MANUALLY\n";
    out << "// Re-generate with: cmake --build build --target GenerateBindings\n";
    out << "//\n// Canonical type names for structural automation. Unlike AllComponents, this\n";
    out << "// visits identity and runtime-only types too, so discovery explains refusals.\n";
    out << "// The boolean says whether the type participates in authored scene copying.\n";
    out << "// Visiting types never constructs component instances.\n";
    out << "template<typename Visitor>\n";
    out << "void VisitComponentTypes(Visitor&& visitor)\n{\n";
    for (const auto& name : componentNames)
    {
        out << "    visitor.template operator()<" << name << ", "
            << (kComponentsNotInTuple.contains(name) ? "false" : "true")
            << ">(\"" << name << "\");\n";
    }
    out << "}\n";
}
                                  ...
                                  const std::string& indent, int depth)
{
    for (auto const& f : fields)
    {
        ...
        const char* elementType = f.isTArray ? "ElementType" : "value_type";
        const char* clearMethod = f.isTArray ? "Empty" : "clear";
        const char* appendMethod = f.isTArray ? "Add" : "push_back";
        ...
        if (f.type == PropType::Struct)
        {
            if (f.isVector)
            {
                ...
                out << indent << "{\n";
                out << indent << "    " << lhs << "." << clearMethod << "();\n";
                out << indent << "    for (auto const& " << el << " : " << sv << ")\n";
                out << indent << "    {\n";
                out << indent << "        decltype(" << lhs << ")::" << elementType << " " << tmp << "{};\n";
                EmitDeserializeFields(out, f.subFields, tmp, el, indent + "        ", depth + 1);
                out << indent << "        " << lhs << "." << appendMethod << "(" << tmp << ");\n";
                out << indent << "    }\n";
                out << indent << "}\n";
            }
            else
        ...
        if (f.isSet)
        {
            ...
            {
                out << indent << "        if (decltype(" << lhs << ")::" << elementType << " v{}; ::YAML::convert<decltype("
                    << lhs << ")::" << elementType << ">::decode(" << el << ", v))\n";
        ...
        if (f.isMap)
        {
            ...
            else if (f.type == PropType::FString)
            {
                out << indent << "        if (std::string v; ::YAML::convert<std::string>::decode(" << entry << ".second, v))\n";
                out << indent << "            " << lhs << "[k" << depth << "] = v;\n";
            }
            else
            {
        ...
        if (f.isVector)
        {
            ...
            if (f.type == PropType::Float)
            {
                ...
                out << indent << "            " << lhs << "." << appendMethod << "(v);\n";
            }
            else if (f.type == PropType::Enum)
            {
                out << indent << "        if (int ev; ::YAML::convert<int>::decode(" << el << ", ev))\n";
                out << indent << "            " << lhs << "." << appendMethod << "(static_cast<decltype(" << lhs
                    << ")::" << elementType << ">(ev));\n";
            }
            else if (f.type == PropType::FString)
            {
                out << indent << "        if (std::string v; ::YAML::convert<std::string>::decode(" << el << ", v))\n";
                out << indent << "            " << lhs << "." << appendMethod << "(v);\n";
            }
            else
            {
        ...
        switch (f.type)
        {
            ...
            case PropType::String:
                ...
                break;
            case PropType::FString:
                out << indent << lhs << " = " << key << ".as<std::string>(" << lhs << ".ToStdString());\n";
                break;
            default:
                break;
        }
                                  ...
                                  const std::string& inst, const std::string& indent, int depth)
{
    for (auto const& f : fields)
    {
        const char* sizeMethod = f.isTArray ? "Num" : "size";
        const std::string access = inst + "." + f.member;
        if (f.type == PropType::Struct)
        {
            if (f.isVector)
            {
                ...
                out << indent << "SceneBinIO::WriteU32(out, static_cast<u32>(" << access << "." << sizeMethod << "()));\n";
                out << indent << "for (auto const& " << ev << " : " << access << ")\n";
                out << indent << "{\n";
        ...
        if (f.isMap)
        {
            ...
            out << indent << "{\n";
            out << indent << "    std::vector<std::string> " << keys << ";\n";
            out << indent << "    " << keys << ".reserve(" << access << "." << sizeMethod << "());\n";
            out << indent << "    for (auto const& " << entry << " : " << access << ")\n";
            out << indent << "        " << keys << ".push_back(" << entry << ".first);\n";
            out << indent << "    std::ranges::sort(" << keys << ");\n";
                                 ...
                                 const std::string& inst, const std::string& indent, int depth)
{
    for (auto const& f : fields)
    {
        const char* elementType = f.isTArray ? "ElementType" : "value_type";
        const char* clearMethod = f.isTArray ? "Empty" : "clear";
        const char* reserveMethod = f.isTArray ? "Reserve" : "reserve";
        const char* appendMethod = f.isTArray ? "Add" : "push_back";
        const std::string lhs = inst + "." + f.member;
        if (f.type == PropType::Struct)
        {
        ...
        if (f.isSet)
        {
            ...
            out << indent << "    for (u32 bi" << depth << " = 0; bi" << depth << " < " << n << "; ++bi" << depth << ")\n";
            out << indent << "    {\n";
            out << indent << "        decltype(" << lhs << ")::" << elementType << " " << v << "{};\n";
            out << indent << "        if (!SceneBinIO::Read(reader, " << v << ")) return false;\n";
        ...
        if (f.isVector)
        {
            ...
            out << indent << "        decltype(" << lhs << ")::" << elementType << " " << v << "{};\n";
            out << indent << "        if (!SceneBinIO::Read(reader, " << v << ")) return false;\n";
            out << indent << "        " << lhs << "." << appendMethod << "(std::move(" << v << "));\n";
            out << indent << "    }\n";
            out << indent << "}\n";
            continue;
        }
...
// from the SAME full data-member scan (ClassifyComponents) that drives the
...
// Registry entries stop accumulating in the current chunk function once it holds
// this many; the next component block opens a new one. ~1600 entries / 48 ≈ 34
// chunk functions of a size clang-cl+ASan digests comfortably.
static constexpr std::size_t kMcpChunkMaxEntries = 48;

// Emits the registry as CHUNKED static functions plus a BuildRegistryChunks
// driver, instead of one flat function body. The flat form — ~1600 closure-heavy
// push_backs in a single BuildRegistry() — ran clang-cl Release+ASan out of
// memory on CI ("paging file too small"): compiler peak memory scales with the
...
class ChunkWriter
{
  public:
    ...
    void AddBlock(const std::string& blockText, std::size_t entryCount)
    {
        if (!m_Open || m_ChunkEntries >= kMcpChunkMaxEntries)
        {
            CloseChunk();
        ...
        m_Out << blockText;
        m_ChunkEntries += entryCount;
    }

    // Close the last chunk and emit the driver the including TU calls.
    void Finish()
    {
        CloseChunk();
...
static bool WriteComponentTypeVisitor(const fs::path& sceneOutDir, const std::set<std::string>& componentStructs)
{
    std::ostringstream output;
    EmitComponentTypeVisitor(output, componentStructs);
    return ReportWrite(sceneOutDir / "ComponentTypes.Generated.inl", output.str());
}
                                  ...
                                  const std::vector<ComponentDef>& properties)
{
    ...
    emitted += EmitMcpSetterFields(writer, properties, componentFields);
    writer.Finish();
    if (emitted == 0)
    {
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if (!WriteAllComponentsTuple(sceneOutDir, componentStructs))
            errors = true;
        if (!WriteComponentTypeVisitor(sceneOutDir, componentStructs))
            errors = true;
        ...
        // full data-member scan (ClassifyComponents), not the OLO_PROPERTY scan
        ...
        for (const auto& headerPath : ScannedHeaders(scanDir))
        {
            std::ifstream file(headerPath);
            if (file.is_open())
                headerContents.emplace_back(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>());
        }
        std::vector<std::string_view> headerSources;
        headerSources.reserve(headerContents.size());
        for (const auto& content : headerContents)
            headerSources.emplace_back(content);
        const auto componentFields = OloHeaderTool::ClassifyComponents(headerSources);
