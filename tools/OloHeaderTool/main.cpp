...
{
    return t == PropType::Float || t == PropType::Bool || t == PropType::Int;
}
...
static std::vector<ComponentDef> ParseHeaders(const fs::path& scanDir)
{
    ...
    for (auto const& entry : fs::recursive_directory_iterator(scanDir))
    {
        ...
        // Map component name → ComponentDef
        std::map<std::string, ComponentDef*> compMap;
...
int main(int argc, char* argv[])
{
    ...
    if (components.empty())
    {
        std::cout << "OloHeaderTool: No OLO_PROPERTY() annotations found.\n";
        return 0;
    }
